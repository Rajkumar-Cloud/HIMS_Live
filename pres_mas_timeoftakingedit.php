...
var fpres_mas_timeoftakingedit = currentForm = new ew.Form("fpres_mas_timeoftakingedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($pres_mas_timeoftaking_edit->ID->Required) { ?>
			...
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pres_mas_timeoftaking->ID->caption(), $pres_mas_timeoftaking->ID->RequiredErrorMessage)) ?>");
		<?php } ?>
...
fpres_mas_timeoftakingedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php if ($pres_mas_timeoftaking_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $pres_mas_timeoftaking_edit->Token ?>">
<?php } ?>
...
<?php if ($pres_mas_timeoftaking->ID->Visible) { // ID ?>
	<div id="r_ID" class="form-group row">
		<label id="elh_pres_mas_timeoftaking_ID" class="<?php echo $pres_mas_timeoftaking_edit->LeftColumnClass ?>"><?php echo $pres_mas_timeoftaking->ID->caption() ?><?php echo ($pres_mas_timeoftaking->ID->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_mas_timeoftaking_edit->RightColumnClass ?>"><div<?php echo $pres_mas_timeoftaking->ID->cellAttributes() ?>>
<span id="el_pres_mas_timeoftaking_ID">
<span<?php echo $pres_mas_timeoftaking->ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($pres_mas_timeoftaking->ID->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="pres_mas_timeoftaking" data-field="x_ID" name="x_ID" id="x_ID" value="<?php echo HtmlEncode($pres_mas_timeoftaking->ID->CurrentValue) ?>">
<?php echo $pres_mas_timeoftaking->ID->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pres_mas_timeoftaking->Time_Of_Taking->Visible) { // Time Of Taking ?>
	<div id="r_Time_Of_Taking" class="form-group row">
		<label id="elh_pres_mas_timeoftaking_Time_Of_Taking" for="x_Time_Of_Taking" class="<?php echo $pres_mas_timeoftaking_edit->LeftColumnClass ?>"><?php echo $pres_mas_timeoftaking->Time_Of_Taking->caption() ?><?php echo ($pres_mas_timeoftaking->Time_Of_Taking->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_mas_timeoftaking_edit->RightColumnClass ?>"><div<?php echo $pres_mas_timeoftaking->Time_Of_Taking->cellAttributes() ?>>
