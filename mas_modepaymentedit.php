...
var fmas_modepaymentedit = currentForm = new ew.Form("fmas_modepaymentedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($mas_modepayment_edit->id->Required) { ?>
			...
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $mas_modepayment->id->caption(), $mas_modepayment->id->RequiredErrorMessage)) ?>");
		<?php } ?>
...
fmas_modepaymentedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmas_modepaymentedit.lists["x_BankingDatails"] = <?php echo $mas_modepayment_edit->BankingDatails->Lookup->toClientList() ?>;
...
<?php if ($mas_modepayment_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $mas_modepayment_edit->Token ?>">
<?php } ?>
...
<?php if ($mas_modepayment->id->Visible) { // id ?>
	<div id="r_id" class="form-group row">
		<label id="elh_mas_modepayment_id" class="<?php echo $mas_modepayment_edit->LeftColumnClass ?>"><?php echo $mas_modepayment->id->caption() ?><?php echo ($mas_modepayment->id->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $mas_modepayment_edit->RightColumnClass ?>"><div<?php echo $mas_modepayment->id->cellAttributes() ?>>
<span id="el_mas_modepayment_id">
<span<?php echo $mas_modepayment->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($mas_modepayment->id->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="mas_modepayment" data-field="x_id" name="x_id" id="x_id" value="<?php echo HtmlEncode($mas_modepayment->id->CurrentValue) ?>">
<?php echo $mas_modepayment->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($mas_modepayment->Mode->Visible) { // Mode ?>
	<div id="r_Mode" class="form-group row">
		<label id="elh_mas_modepayment_Mode" for="x_Mode" class="<?php echo $mas_modepayment_edit->LeftColumnClass ?>"><?php echo $mas_modepayment->Mode->caption() ?><?php echo ($mas_modepayment->Mode->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $mas_modepayment_edit->RightColumnClass ?>"><div<?php echo $mas_modepayment->Mode->cellAttributes() ?>>
...
<?php if ($mas_modepayment->BankingDatails->Visible) { // BankingDatails ?>
	<div id="r_BankingDatails" class="form-group row">
		<label id="elh_mas_modepayment_BankingDatails" for="x_BankingDatails" class="<?php echo $mas_modepayment_edit->LeftColumnClass ?>"><?php echo $mas_modepayment->BankingDatails->caption() ?><?php echo ($mas_modepayment->BankingDatails->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $mas_modepayment_edit->RightColumnClass ?>"><div<?php echo $mas_modepayment->BankingDatails->cellAttributes() ?>>
