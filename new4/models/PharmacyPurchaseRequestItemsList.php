 ...
 * Page class
 */
class PharmacyPurchaseRequestItemsList extends PharmacyPurchaseRequestItems
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PharmacyPurchaseRequestItemsList";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PharmacyPurchaseRequestItemsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PharmacyPurchaseRequestItemsDelete";
        $this->MultiUpdateUrl = "PharmacyPurchaseRequestItemsUpdate";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("pharmacy_purchase_request_items"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pharmacy_purchase_request") {
            $masterTbl = Container("pharmacy_purchase_request");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("PharmacyPurchaseRequestList"); // Return to master page
                return;
            } else {
    ...
    protected function validateForm()
    {
        ...
        if ($this->prid->Required) {
            if (!$this->prid->IsDetailKey && EmptyValue($this->prid->FormValue)) {
                $this->prid->addErrorMessage(str_replace("%s", $this->prid->caption(), $this->prid->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->prid->FormValue)) {
            $this->prid->addErrorMessage($this->prid->getErrorMessage(false));
        }
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['prid']); // prid
        return md5($hash);
    }
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pharmacy_purchase_request") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $pharmacy_purchase_request->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
