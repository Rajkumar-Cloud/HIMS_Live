 ...
 * Page class
 */
class EmployeeAddressList extends EmployeeAddress
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeAddressList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "femployee_addresslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "EmployeeAddressAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "EmployeeAddressDelete";
        $this->MultiUpdateUrl = "EmployeeAddressUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option femployee_addresslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("employee_address"));
                $doc->Text = @$content;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_contact_persion") && $CurrentForm->hasValue("o_contact_persion") && $this->contact_persion->CurrentValue != $this->contact_persion->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_street") && $CurrentForm->hasValue("o_street") && $this->street->CurrentValue != $this->street->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_address_type") && $CurrentForm->hasValue("o_address_type") && $this->address_type->CurrentValue != $this->address_type->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->contact_persion->clearErrorMessage();
        $this->street->clearErrorMessage();
        $this->town->clearErrorMessage();
        $this->province->clearErrorMessage();
        $this->postal_code->clearErrorMessage();
        $this->address_type->clearErrorMessage();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "femployee_addresslistsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->employee_id->AdvancedSearch->toJson(), ","); // Field employee_id
        $filterList = Concat($filterList, $this->contact_persion->AdvancedSearch->toJson(), ","); // Field contact_persion
        ...
        $filterList = Concat($filterList, $this->address_type->AdvancedSearch->toJson(), ","); // Field address_type
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "femployee_addresslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->contact_persion->AdvancedSearch->SearchValue = @$filter["x_contact_persion"];
        $this->contact_persion->AdvancedSearch->SearchOperator = @$filter["z_contact_persion"];
        $this->contact_persion->AdvancedSearch->SearchCondition = @$filter["v_contact_persion"];
        $this->contact_persion->AdvancedSearch->SearchValue2 = @$filter["y_contact_persion"];
        $this->contact_persion->AdvancedSearch->SearchOperator2 = @$filter["w_contact_persion"];
        $this->contact_persion->AdvancedSearch->save();
        ...
        // Field address_type
        $this->address_type->AdvancedSearch->SearchValue = @$filter["x_address_type"];
        $this->address_type->AdvancedSearch->SearchOperator = @$filter["z_address_type"];
        $this->address_type->AdvancedSearch->SearchCondition = @$filter["v_address_type"];
        $this->address_type->AdvancedSearch->SearchValue2 = @$filter["y_address_type"];
        $this->address_type->AdvancedSearch->SearchOperator2 = @$filter["w_address_type"];
        $this->address_type->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->contact_persion, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->contact_persion); // contact_persion
            ...
            $this->updateSort($this->address_type); // address_type
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"femployee_addresslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"femployee_addresslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.femployee_addresslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->contact_persion->CurrentValue = null;
        $this->contact_persion->OldValue = $this->contact_persion->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_contact_persion")) {
            $this->contact_persion->setOldValue($CurrentForm->getValue("o_contact_persion"));
        }
        ...
        if ($CurrentForm->hasValue("o_street")) {
            $this->street->setOldValue($CurrentForm->getValue("o_street"));
        }
        ...
        if ($CurrentForm->hasValue("o_town")) {
        ...
        if ($CurrentForm->hasValue("o_province")) {
        ...
        if ($CurrentForm->hasValue("o_address_type")) {
            $this->address_type->setOldValue($CurrentForm->getValue("o_address_type"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['contact_persion'] = $this->contact_persion->CurrentValue;
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->address_type->ViewValue === null) { // Lookup from database
                    ...
                    $sqlWrk = $this->address_type->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->address_type->Lookup->renderViewRow($rswrk[0]);
            ...
            if ($this->address_type->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->address_type->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
    ...
    protected function validateForm()
    {
        ...
        if ($this->contact_persion->Required) {
            if (!$this->contact_persion->IsDetailKey && EmptyValue($this->contact_persion->FormValue)) {
                $this->contact_persion->addErrorMessage(str_replace("%s", $this->contact_persion->caption(), $this->contact_persion->RequiredErrorMessage));
            }
        }
        ...
        if ($this->address_type->Required) {
            if (!$this->address_type->IsDetailKey && EmptyValue($this->address_type->FormValue)) {
                $this->address_type->addErrorMessage(str_replace("%s", $this->address_type->caption(), $this->address_type->RequiredErrorMessage));
            }
        }
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['contact_persion']); // contact_persion
        $hash .= GetFieldHash($row['street']); // street
        $hash .= GetFieldHash($row['town']); // town
        $hash .= GetFieldHash($row['province']); // province
        $hash .= GetFieldHash($row['postal_code']); // postal_code
        $hash .= GetFieldHash($row['address_type']); // address_type
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'employee_address'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_employee();
        ...
        // contact_persion
        $this->contact_persion->setDbValueDef($rsnew, $this->contact_persion->CurrentValue, "", false);
        ...
        // address_type
        $this->address_type->setDbValueDef($rsnew, $this->address_type->CurrentValue, 0, false);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.femployee_addresslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.femployee_addresslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.femployee_addresslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_employee_address" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_employee_address\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.femployee_addresslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"femployee_addresslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
