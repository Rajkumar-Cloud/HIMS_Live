 ...
 * Page class
 */
class EmployeeTimeentryAdd extends EmployeeTimeentry
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeTimeentryAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("EmployeeTimeentryList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "EmployeeTimeentryList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "EmployeeTimeentryView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->timesheet->CurrentValue = null;
        $this->timesheet->OldValue = $this->timesheet->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'timesheet' first before field var 'x_timesheet'
        $val = $CurrentForm->hasValue("timesheet") ? $CurrentForm->getValue("timesheet") : $CurrentForm->getValue("x_timesheet");
        if (!$this->timesheet->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->timesheet->Visible = false; // Disable update for API request
            } else {
                $this->timesheet->setFormValue($val);
            }
        }
        ...
        // Check field name 'time_start' first before field var 'x_time_start'
        $val = $CurrentForm->hasValue("time_start") ? $CurrentForm->getValue("time_start") : $CurrentForm->getValue("x_time_start");
        if (!$this->time_start->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->time_start->Visible = false; // Disable update for API request
            } else {
                $this->time_start->setFormValue($val);
            }
        }
        ...
        // Check field name 'time_end' first before field var 'x_time_end'
        $val = $CurrentForm->hasValue("time_end") ? $CurrentForm->getValue("time_end") : $CurrentForm->getValue("x_time_end");
        if (!$this->time_end->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->time_end->Visible = false; // Disable update for API request
            } else {
                $this->time_end->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->timesheet->CurrentValue = $this->timesheet->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['project'] = $this->project->CurrentValue;
        $row['employee'] = $this->employee->CurrentValue;
        $row['timesheet'] = $this->timesheet->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->timesheet->EditValue = HtmlEncode($this->timesheet->CurrentValue);
            ...
            $this->time_start->EditValue = HtmlEncode($this->time_start->CurrentValue);
            ...
            $this->time_end->EditValue = HtmlEncode($this->time_end->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->project->Required) {
            if (!$this->project->IsDetailKey && EmptyValue($this->project->FormValue)) {
                $this->project->addErrorMessage(str_replace("%s", $this->project->caption(), $this->project->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->project->FormValue)) {
            $this->project->addErrorMessage($this->project->getErrorMessage(false));
        }
        ...
        if ($this->timesheet->Required) {
            if (!$this->timesheet->IsDetailKey && EmptyValue($this->timesheet->FormValue)) {
                $this->timesheet->addErrorMessage(str_replace("%s", $this->timesheet->caption(), $this->timesheet->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->timesheet->FormValue)) {
            $this->timesheet->addErrorMessage($this->timesheet->getErrorMessage(false));
        }
        ...
        if ($this->time_start->Required) {
            if (!$this->time_start->IsDetailKey && EmptyValue($this->time_start->FormValue)) {
                $this->time_start->addErrorMessage(str_replace("%s", $this->time_start->caption(), $this->time_start->RequiredErrorMessage));
            }
        }
        ...
        if ($this->time_end->Required) {
            if (!$this->time_end->IsDetailKey && EmptyValue($this->time_end->FormValue)) {
                $this->time_end->addErrorMessage(str_replace("%s", $this->time_end->caption(), $this->time_end->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // project
        $this->project->setDbValueDef($rsnew, $this->project->CurrentValue, null, false);

        // employee
        $this->employee->setDbValueDef($rsnew, $this->employee->CurrentValue, 0, false);

        // timesheet
        $this->timesheet->setDbValueDef($rsnew, $this->timesheet->CurrentValue, 0, false);
        ...
        // time_start
        $this->time_start->setDbValueDef($rsnew, $this->time_start->CurrentValue, "", false);

        // date_end
        $this->date_end->setDbValueDef($rsnew, UnFormatDateTime($this->date_end->CurrentValue, 0), null, false);

        // time_end
        $this->time_end->setDbValueDef($rsnew, $this->time_end->CurrentValue, "", false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("EmployeeTimeentryList"), "", $this->TableVar, true);
