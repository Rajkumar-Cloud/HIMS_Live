 ...
 * Page class
 */
class ViewLabServicesAuthView extends ViewLabServicesAuth
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewLabServicesAuthView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "ViewLabServicesAuthList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ViewLabServicesAuthList"; // No matching record, return to list
                    }
                    break;
            }
            ...
            $returnUrl = "ViewLabServicesAuthList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ViewLabResultreleasedAuthList?" . Config("TABLE_SHOW_MASTER") . "=view_lab_services_auth&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_lab_services_auth')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_lab_resultreleased_auth"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("view_lab_resultreleased_auth", explode(",", $this->getCurrentDetailTable()))) {
            $view_lab_resultreleased_auth = Container("view_lab_resultreleased_auth");
            ...
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $view_lab_resultreleased_auth->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
