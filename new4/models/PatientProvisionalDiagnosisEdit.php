 ...
 * Page class
 */
class PatientProvisionalDiagnosisEdit extends PatientProvisionalDiagnosis
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PatientProvisionalDiagnosisEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("PatientProvisionalDiagnosisList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "PatientProvisionalDiagnosisList") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->provisionaldiagnosisTemplate->ViewValue !== null) { // Load from cache
                ...
                $lookupFilter = $lookupFilter->bindTo($this);
                $sqlWrk = $this->provisionaldiagnosisTemplate->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->provisionaldiagnosisTemplate->EditValue = $arwrk;
            }
            $this->provisionaldiagnosisTemplate->PlaceHolder = RemoveHtml($this->provisionaldiagnosisTemplate->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->provisionaldiagnosisTemplate->Required) {
            if (!$this->provisionaldiagnosisTemplate->IsDetailKey && EmptyValue($this->provisionaldiagnosisTemplate->FormValue)) {
                $this->provisionaldiagnosisTemplate->addErrorMessage(str_replace("%s", $this->provisionaldiagnosisTemplate->caption(), $this->provisionaldiagnosisTemplate->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // provisional_diagnosis
            $this->provisional_diagnosis->setDbValueDef($rsnew, $this->provisional_diagnosis->CurrentValue, null, $this->provisional_diagnosis->ReadOnly);
            ...
            // provisionaldiagnosisTemplate
            $this->provisionaldiagnosisTemplate->setDbValueDef($rsnew, $this->provisionaldiagnosisTemplate->CurrentValue, "", $this->provisionaldiagnosisTemplate->ReadOnly);
