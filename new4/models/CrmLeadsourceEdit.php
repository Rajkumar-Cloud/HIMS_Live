 ...
 * Page class
 */
class CrmLeadsourceEdit extends CrmLeadsource
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CrmLeadsourceEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("leadsourceid") ?? Key(0) ?? Route(2)) !== null) {
                $this->leadsourceid->setQueryStringValue($keyValue);
                $this->leadsourceid->setOldValue($this->leadsourceid->QueryStringValue);
            } elseif (Post("leadsourceid") !== null) {
                $this->leadsourceid->setFormValue(Post("leadsourceid"));
                $this->leadsourceid->setOldValue($this->leadsourceid->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("CrmLeadsourceList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "CrmLeadsourceList") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->leadsourceid->Required) {
            if (!$this->leadsourceid->IsDetailKey && EmptyValue($this->leadsourceid->FormValue)) {
                $this->leadsourceid->addErrorMessage(str_replace("%s", $this->leadsourceid->caption(), $this->leadsourceid->RequiredErrorMessage));
            }
        }
