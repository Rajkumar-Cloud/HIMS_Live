...
class IpAdmissionEdit extends IpAdmission
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_history", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientProvisionalDiagnosisGrid");
        if (in_array("patient_provisional_diagnosis", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientPrescriptionGrid");
        if (in_array("patient_prescription", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientFinalDiagnosisGrid");
        if (in_array("patient_final_diagnosis", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientFollowUpGrid");
        if (in_array("patient_follow_up", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientOtDeliveryRegisterGrid");
        if (in_array("patient_ot_delivery_register", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientOtSurgeryRegisterGrid");
        if (in_array("patient_ot_surgery_register", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        ...
        if (in_array("patient_investigations", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientInsuranceGrid");
        if (in_array("patient_insurance", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PharmacyPharledGrid");
        if (in_array("pharmacy_pharled", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("ViewIpAdvanceGrid");
        if (in_array("view_ip_advance", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientRoomGrid");
        if (in_array("patient_room", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
