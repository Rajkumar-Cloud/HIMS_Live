 ...
 * Page class
 */
class EmployeeDependentsAdd extends EmployeeDependents
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeDependentsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("EmployeeDependentsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "EmployeeDependentsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "EmployeeDependentsView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->id_number->CurrentValue = null;
        $this->id_number->OldValue = $this->id_number->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'id_number' first before field var 'x_id_number'
        $val = $CurrentForm->hasValue("id_number") ? $CurrentForm->getValue("id_number") : $CurrentForm->getValue("x_id_number");
        if (!$this->id_number->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->id_number->Visible = false; // Disable update for API request
            } else {
                $this->id_number->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->dob->CurrentValue = UnFormatDateTime($this->dob->CurrentValue, 0);
        $this->id_number->CurrentValue = $this->id_number->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['id_number'] = $this->id_number->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->id_number->EditValue = HtmlEncode($this->id_number->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->relationship->Required) {
            if ($this->relationship->FormValue == "") {
        ...
        if ($this->id_number->Required) {
            if (!$this->id_number->IsDetailKey && EmptyValue($this->id_number->FormValue)) {
                $this->id_number->addErrorMessage(str_replace("%s", $this->id_number->caption(), $this->id_number->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // id_number
        $this->id_number->setDbValueDef($rsnew, $this->id_number->CurrentValue, null, false);
