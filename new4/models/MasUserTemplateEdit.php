 ...
 * Page class
 */
class MasUserTemplateEdit extends MasUserTemplate
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MasUserTemplateEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("MasUserTemplateList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "MasUserTemplateList") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->TemplateType->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->TemplateType->Visible = false; // Disable update for API request
            } else {
                $this->TemplateType->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->TemplateType->CurrentValue = $this->TemplateType->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->TemplateType->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->TemplateType->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
    ...
    protected function validateForm()
    {
        ...
        if ($this->TemplateType->Required) {
            if (!$this->TemplateType->IsDetailKey && EmptyValue($this->TemplateType->FormValue)) {
                $this->TemplateType->addErrorMessage(str_replace("%s", $this->TemplateType->caption(), $this->TemplateType->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // TemplateType
            $this->TemplateType->setDbValueDef($rsnew, $this->TemplateType->CurrentValue, "", $this->TemplateType->ReadOnly);

            // Template
            $this->_Template->setDbValueDef($rsnew, $this->_Template->CurrentValue, null, $this->_Template->ReadOnly);
