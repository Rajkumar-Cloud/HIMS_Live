 ...
 * Page class
 */
class ViewPharmacyMovementItemList extends ViewPharmacyMovementItem
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewPharmacyMovementItemList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "ViewPharmacyMovementItemDelete";
        $this->MultiUpdateUrl = "ViewPharmacyMovementItemUpdate";
    ...
    protected function setupSearchOptions()
    {
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"ViewPharmacyMovementItemSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"view_pharmacy_movement_item\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'ViewPharmacyMovementItemSearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "view_pharmacy_movement") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $view_pharmacy_movement->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "view_pharmacy_movement") {
                ...
                if (($parm = Get("fk_prc", Get("prc"))) !== null) {
                    $masterTbl->prc->setQueryStringValue($parm);
                    $this->prc->setQueryStringValue($masterTbl->prc->QueryStringValue);
                    $this->prc->setSessionValue($this->prc->QueryStringValue);
                } else {
                    $validMaster = false;
                }
                if (($parm = Get("fk_BatchNo", Get("BatchNo"))) !== null) {
                    $masterTbl->BatchNo->setQueryStringValue($parm);
                    $this->BatchNo->setQueryStringValue($masterTbl->BatchNo->QueryStringValue);
            ...
            if ($masterTblVar == "view_pharmacy_movement") {
                ...
                if (($parm = Post("fk_prc", Post("prc"))) !== null) {
                    $masterTbl->prc->setFormValue($parm);
                    $this->prc->setFormValue($masterTbl->prc->FormValue);
                    $this->prc->setSessionValue($this->prc->FormValue);
                } else {
                    $validMaster = false;
                }
                if (($parm = Post("fk_BatchNo", Post("BatchNo"))) !== null) {
                    $masterTbl->BatchNo->setFormValue($parm);
                    $this->BatchNo->setFormValue($masterTbl->BatchNo->FormValue);
