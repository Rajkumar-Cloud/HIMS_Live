 ...
 * Page class
 */
class LabProfileDetailsList extends LabProfileDetails
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LabProfileDetailsList";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "LabProfileDetailsAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "LabProfileDetailsDelete";
        $this->MultiUpdateUrl = "LabProfileDetailsUpdate";
    ...
    public function run()
    {
        ...
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "view_lab_profile") {
            ...
            if (!$this->MasterRecordExists) {
                ...
                $this->terminate("ViewLabProfileList"); // Return to master page
                return;
            } else {
    ...
    public function resetFormError()
    {
        ...
        $this->ProfileCode->clearErrorMessage();
        $this->SubProfileCode->clearErrorMessage();
        $this->ProfileTestCode->clearErrorMessage();
        $this->ProfileSubTestCode->clearErrorMessage();
        $this->TestOrder->clearErrorMessage();
        $this->TestAmount->clearErrorMessage();
    }
    ...
    protected function loadSortOrder()
    {
        ...
        if ($orderBy == "") {
            $this->DefaultSort = "`TestOrder` ASC";
            if ($this->getSqlOrderBy() != "") {
                $useDefaultSort = true;
                if ($this->TestOrder->getSort() != "") {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['ProfileCode']); // ProfileCode
        $hash .= GetFieldHash($row['SubProfileCode']); // SubProfileCode
        $hash .= GetFieldHash($row['ProfileTestCode']); // ProfileTestCode
        $hash .= GetFieldHash($row['ProfileSubTestCode']); // ProfileSubTestCode
        $hash .= GetFieldHash($row['TestOrder']); // TestOrder
        $hash .= GetFieldHash($row['TestAmount']); // TestAmount
        return md5($hash);
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"LabProfileDetailsSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"lab_profile_details\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'LabProfileDetailsSearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "view_lab_profile") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $view_lab_profile->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
