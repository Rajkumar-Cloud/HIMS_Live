 ...
 * Page class
 */
class PharmacyPharledList extends PharmacyPharled
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PharmacyPharledList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "PharmacyPharledDelete";
        $this->MultiUpdateUrl = "PharmacyPharledUpdate";
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['baid']); // baid
        $hash .= GetFieldHash($row['isdate']); // isdate
        ...
        $hash .= GetFieldHash($row['HSNCODE']); // HSNCODE
        return md5($hash);
    }
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pharmacy_billing_voucher") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $pharmacy_billing_voucher->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "pharmacy_billing_issue") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $pharmacy_billing_issue->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
                $doc->setStyle($exportStyle); // Restore
