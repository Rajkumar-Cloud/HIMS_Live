 ...
 * Page class
 */
class HospitalAdd extends Hospital
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HospitalAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("HospitalList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "HospitalList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "HospitalView") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->hospital->Required) {
            if (!$this->hospital->IsDetailKey && EmptyValue($this->hospital->FormValue)) {
                $this->hospital->addErrorMessage(str_replace("%s", $this->hospital->caption(), $this->hospital->RequiredErrorMessage));
            }
        }
        ...
        if ($this->BillingGST->Required) {
            if (!$this->BillingGST->IsDetailKey && EmptyValue($this->BillingGST->FormValue)) {
                $this->BillingGST->addErrorMessage(str_replace("%s", $this->BillingGST->caption(), $this->BillingGST->RequiredErrorMessage));
            }
        }
