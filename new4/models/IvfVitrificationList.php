 ...
 * Page class
 */
class IvfVitrificationList extends IvfVitrification
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "IvfVitrificationList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fivf_vitrificationlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "IvfVitrificationAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "IvfVitrificationDelete";
        $this->MultiUpdateUrl = "IvfVitrificationUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fivf_vitrificationlistsrch";
    ...
    public function resetFormError()
    {
        ...
        $this->FertilisationDate->clearErrorMessage();
        ...
        $this->Gobiet->clearErrorMessage();
        $this->CryolockNo->clearErrorMessage();
        $this->CryolockColour->clearErrorMessage();
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fivf_vitrificationlistsrch");
        }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fivf_vitrificationlistsrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Gobiet); // Gobiet
            $this->updateSort($this->CryolockNo); // CryolockNo
            $this->updateSort($this->CryolockColour); // CryolockColour
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fivf_vitrificationlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fivf_vitrificationlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fivf_vitrificationlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_FertilisationDate")) {
        ...
        if ($CurrentForm->hasValue("o_Gobiet")) {
        ...
        if ($CurrentForm->hasValue("o_CryolockNo")) {
        ...
        if ($CurrentForm->hasValue("o_CryolockColour")) {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['PrimaryEmbryologist']); // PrimaryEmbryologist
        $hash .= GetFieldHash($row['SecondaryEmbryologist']); // SecondaryEmbryologist
        $hash .= GetFieldHash($row['EmbryoNo']); // EmbryoNo
        $hash .= GetFieldHash($row['FertilisationDate']); // FertilisationDate
        $hash .= GetFieldHash($row['Day']); // Day
        $hash .= GetFieldHash($row['Grade']); // Grade
        $hash .= GetFieldHash($row['Incubator']); // Incubator
        $hash .= GetFieldHash($row['Tank']); // Tank
        $hash .= GetFieldHash($row['Canister']); // Canister
        $hash .= GetFieldHash($row['Gobiet']); // Gobiet
        $hash .= GetFieldHash($row['CryolockNo']); // CryolockNo
        $hash .= GetFieldHash($row['CryolockColour']); // CryolockColour
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fivf_vitrificationlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fivf_vitrificationlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fivf_vitrificationlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_ivf_vitrification" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_ivf_vitrification\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fivf_vitrificationlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fivf_vitrificationlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
