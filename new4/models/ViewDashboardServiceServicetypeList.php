 ...
 * Page class
 */
class ViewDashboardServiceServicetypeList extends ViewDashboardServiceServicetype
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewDashboardServiceServicetypeList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "ViewDashboardServiceServicetypeDelete";
        $this->MultiUpdateUrl = "ViewDashboardServiceServicetypeUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->sumSubTotal->AdvancedSearch->toJson(), ","); // Field sum(SubTotal)
    ...
    protected function restoreFilterList()
    {
        ...
        // Field sum(SubTotal)
        $this->sumSubTotal->AdvancedSearch->SearchValue = @$filter["x_sumSubTotal"];
        $this->sumSubTotal->AdvancedSearch->SearchOperator = @$filter["z_sumSubTotal"];
        $this->sumSubTotal->AdvancedSearch->SearchCondition = @$filter["v_sumSubTotal"];
        $this->sumSubTotal->AdvancedSearch->SearchValue2 = @$filter["y_sumSubTotal"];
        $this->sumSubTotal->AdvancedSearch->SearchOperator2 = @$filter["w_sumSubTotal"];
        $this->sumSubTotal->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->sumSubTotal, $default, false); // sum(SubTotal)
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->sumSubTotal->AdvancedSearch->save(); // sum(SubTotal)
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->sumSubTotal->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->sumSubTotal->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->sumSubTotal->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->sumSubTotal); // sum(SubTotal)
    ...
    public function renderListOptions()
    {
        ...
        // "detail_view_dashboard_service_details"
        $opt = $this->ListOptions["detail_view_dashboard_service_details"];
        if ($Security->allowList(CurrentProjectID() . 'view_dashboard_service_details')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("ViewDashboardServiceDetailsList?" . Config("TABLE_SHOW_MASTER") . "=view_dashboard_service_servicetype&" . GetForeignKeyUrl("fk_DEPARTMENT", $this->DEPARTMENT->CurrentValue) . "&" . GetForeignKeyUrl("fk_HospID", $this->HospID->CurrentValue) . "&" . GetForeignKeyUrl("fk_SERVICE_TYPE", $this->SERVICE_TYPE->CurrentValue) . "&" . GetForeignKeyUrl("fk_createdDate", $this->createdDate->CurrentValue, 7) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewDashboardServiceDetailsGrid");
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_dashboard_service_servicetypelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_dashboard_service_servicetypelistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_dashboard_service_servicetypelist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // sum(SubTotal)
        if (!$this->isAddOrEdit() && $this->sumSubTotal->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->sumSubTotal->AdvancedSearch->SearchValue != "" || $this->sumSubTotal->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->sumSubTotal->EditValue = HtmlEncode($this->sumSubTotal->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fview_dashboard_service_servicetypelist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fview_dashboard_service_servicetypelist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fview_dashboard_service_servicetypelist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_dashboard_service_servicetypelistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "view_dashboard_service_summary") {
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    ...
                    $view_dashboard_service_summary->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
