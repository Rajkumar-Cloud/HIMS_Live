...
class PatientAdd extends Patient
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("patient_address", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientEmailGrid");
        if (in_array("patient_email", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientTelephoneGrid");
        if (in_array("patient_telephone", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientEmergencyContactGrid");
        if (in_array("patient_emergency_contact", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("PatientDocumentGrid");
        if (in_array("patient_document", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
