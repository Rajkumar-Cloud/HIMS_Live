 ...
 */
class ViewFindDiffBills extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // sum(b.amount)
        $this->sumbamount = new DbField('view_find_diff_bills', 'view_find_diff_bills', 'x_sumbamount', 'sum(b.amount)', '`sum(b.amount)`', '`sum(b.amount)`', 131, 34, -1, false, '`sum(b.amount)`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->sumbamount->Sortable = true; // Allow sort
        $this->sumbamount->DefaultDecimalPrecision = 2; // Default decimal precision
        $this->sumbamount->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->sumbamount->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->sumbamount->Param, "CustomMsg");
        $this->Fields['sum(b.amount)'] = &$this->sumbamount;
        ...
        $this->diff->DefaultDecimalPrecision = 2; // Default decimal precision
        $this->diff->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->diff->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->diff->Param, "CustomMsg");
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->sumbamount->DbValue = $row['sum(b.amount)'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewFindDiffBillsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "ViewFindDiffBillsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "ViewFindDiffBillsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "ViewFindDiffBillsAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "ViewFindDiffBillsView";
            case Config("API_ADD_ACTION"):
                return "ViewFindDiffBillsAdd";
            case Config("API_EDIT_ACTION"):
                return "ViewFindDiffBillsEdit";
            case Config("API_DELETE_ACTION"):
                return "ViewFindDiffBillsDelete";
            case Config("API_LIST_ACTION"):
                return "ViewFindDiffBillsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewFindDiffBillsView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("ViewFindDiffBillsView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "ViewFindDiffBillsAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "ViewFindDiffBillsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFindDiffBillsEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFindDiffBillsAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("ViewFindDiffBillsDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->sumbamount->EditValue = $this->sumbamount->CurrentValue;
        $this->sumbamount->PlaceHolder = RemoveHtml($this->sumbamount->caption());
        if (strval($this->sumbamount->EditValue) != "" && is_numeric($this->sumbamount->EditValue)) {
            $this->sumbamount->EditValue = FormatNumber($this->sumbamount->EditValue, -2, -2, -2, -2);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->sumbamount);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->sumbamount);
