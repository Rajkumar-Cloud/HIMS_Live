 ...
 * Page class
 */
class ViewAppointmentSchedulerNewUpdate extends ViewAppointmentSchedulerNew
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewAppointmentSchedulerNewUpdate";
    ...
    public function run()
    {
        ...
        if (count($this->RecKeys) <= 0) {
            $this->terminate("ViewAppointmentSchedulerNewList"); // No records selected, return to list
            return;
        }
        if ($this->isUpdate()) {
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while (!$rs->EOF) {
                if ($i == 1) {
                    ...
                    if (!CompareValue($this->start_time->DbValue, $rs->fields['start_time'])) {
    ...
    protected function validateForm()
    {
        ...
        if ($this->start_time->multiUpdateSelected()) {
            $updateCnt++;
        }
        ...
        if ($this->start_time->Required) {
            if ($this->start_time->MultiUpdate != "" && !$this->start_time->IsDetailKey && EmptyValue($this->start_time->FormValue)) {
        ...
        if ($this->patienttype->Required) {
            if ($this->patienttype->MultiUpdate != "" && !$this->patienttype->IsDetailKey && EmptyValue($this->patienttype->FormValue)) {
