 ...
 * Page class
 */
class ViewPharmacyPharledReturnAdd extends ViewPharmacyPharledReturn
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewPharmacyPharledReturnAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("ViewPharmacyPharledReturnList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ViewPharmacyPharledReturnList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ViewPharmacyPharledReturnView") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->sretid->Required) {
            if (!$this->sretid->IsDetailKey && EmptyValue($this->sretid->FormValue)) {
                $this->sretid->addErrorMessage(str_replace("%s", $this->sretid->caption(), $this->sretid->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->sretid->FormValue)) {
            $this->sretid->addErrorMessage($this->sretid->getErrorMessage(false));
        }
        if ($this->sprid->Required) {
            if (!$this->sprid->IsDetailKey && EmptyValue($this->sprid->FormValue)) {
                $this->sprid->addErrorMessage(str_replace("%s", $this->sprid->caption(), $this->sprid->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->sprid->FormValue)) {
            $this->sprid->addErrorMessage($this->sprid->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // sretid
        $this->sretid->setDbValueDef($rsnew, $this->sretid->CurrentValue, null, false);

        // sprid
        $this->sprid->setDbValueDef($rsnew, $this->sprid->CurrentValue, null, false);
