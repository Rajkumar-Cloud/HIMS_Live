 ...
 * Page class
 */
class CrmCrmentityEdit extends CrmCrmentity
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CrmCrmentityEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("crmid") ?? Key(0) ?? Route(2)) !== null) {
                $this->crmid->setQueryStringValue($keyValue);
                $this->crmid->setOldValue($this->crmid->QueryStringValue);
            } elseif (Post("crmid") !== null) {
                $this->crmid->setFormValue(Post("crmid"));
                $this->crmid->setOldValue($this->crmid->FormValue);
            } else {
    ...
    protected function validateForm()
    {
        ...
        if ($this->crmid->Required) {
            if (!$this->crmid->IsDetailKey && EmptyValue($this->crmid->FormValue)) {
                $this->crmid->addErrorMessage(str_replace("%s", $this->crmid->caption(), $this->crmid->RequiredErrorMessage));
            }
        }
