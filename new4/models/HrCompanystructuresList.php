 ...
 * Page class
 */
class HrCompanystructuresList extends HrCompanystructures
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HrCompanystructuresList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fhr_companystructureslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "HrCompanystructuresAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "HrCompanystructuresDelete";
        $this->MultiUpdateUrl = "HrCompanystructuresUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fhr_companystructureslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("hr_companystructures"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fhr_companystructureslistsrch");
        }
        ...
        $filterList = Concat($filterList, $this->heads->AdvancedSearch->toJson(), ","); // Field heads
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fhr_companystructureslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field heads
        $this->heads->AdvancedSearch->SearchValue = @$filter["x_heads"];
        $this->heads->AdvancedSearch->SearchOperator = @$filter["z_heads"];
        $this->heads->AdvancedSearch->SearchCondition = @$filter["v_heads"];
        $this->heads->AdvancedSearch->SearchValue2 = @$filter["y_heads"];
        $this->heads->AdvancedSearch->SearchOperator2 = @$filter["w_heads"];
        $this->heads->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->heads, $default, false); // heads
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->heads->AdvancedSearch->save(); // heads
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->heads, $arKeywords, $type);
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->heads->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
                ...
                $this->heads->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        // Restore advanced search values
                ...
                $this->heads->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->heads); // heads
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhr_companystructureslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhr_companystructureslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fhr_companystructureslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadSearchValues()
    {
        ...
        // timezone
        if (!$this->isAddOrEdit() && $this->timezone->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->timezone->AdvancedSearch->SearchValue != "" || $this->timezone->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // heads
        if (!$this->isAddOrEdit() && $this->heads->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->heads->AdvancedSearch->SearchValue != "" || $this->heads->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->parent->EditValue = HtmlEncode($this->parent->AdvancedSearch->SearchValue);
            ...
            if (!$this->timezone->Raw) {
                $this->timezone->AdvancedSearch->SearchValue = HtmlDecode($this->timezone->AdvancedSearch->SearchValue);
            }
            $this->timezone->EditValue = HtmlEncode($this->timezone->AdvancedSearch->SearchValue);
            ...
            if (!$this->heads->Raw) {
                $this->heads->AdvancedSearch->SearchValue = HtmlDecode($this->heads->AdvancedSearch->SearchValue);
            }
            $this->heads->EditValue = HtmlEncode($this->heads->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fhr_companystructureslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fhr_companystructureslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fhr_companystructureslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_hr_companystructures" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_hr_companystructures\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fhr_companystructureslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhr_companystructureslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
