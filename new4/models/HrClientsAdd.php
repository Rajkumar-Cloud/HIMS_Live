 ...
 * Page class
 */
class HrClientsAdd extends HrClients
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "HrClientsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("HrClientsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "HrClientsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "HrClientsView") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->first_contact_date->Required) {
            if (!$this->first_contact_date->IsDetailKey && EmptyValue($this->first_contact_date->FormValue)) {
                $this->first_contact_date->addErrorMessage(str_replace("%s", $this->first_contact_date->caption(), $this->first_contact_date->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->first_contact_date->FormValue)) {
            $this->first_contact_date->addErrorMessage($this->first_contact_date->getErrorMessage(false));
        }
        ...
        if ($this->company_url->Required) {
            if (!$this->company_url->IsDetailKey && EmptyValue($this->company_url->FormValue)) {
                $this->company_url->addErrorMessage(str_replace("%s", $this->company_url->caption(), $this->company_url->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("HrClientsList"), "", $this->TableVar, true);
