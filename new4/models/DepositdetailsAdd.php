 ...
 * Page class
 */
class DepositdetailsAdd extends Depositdetails
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DepositdetailsAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "DepositdetailsView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->ManualCash->CurrentValue = null;
        $this->ManualCash->OldValue = $this->ManualCash->CurrentValue;
        $this->ManualCard->CurrentValue = null;
        $this->ManualCard->OldValue = $this->ManualCard->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ManualCash' first before field var 'x_ManualCash'
        $val = $CurrentForm->hasValue("ManualCash") ? $CurrentForm->getValue("ManualCash") : $CurrentForm->getValue("x_ManualCash");
        if (!$this->ManualCash->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ManualCash->Visible = false; // Disable update for API request
            } else {
                $this->ManualCash->setFormValue($val);
            }
        }

        // Check field name 'ManualCard' first before field var 'x_ManualCard'
        $val = $CurrentForm->hasValue("ManualCard") ? $CurrentForm->getValue("ManualCard") : $CurrentForm->getValue("x_ManualCard");
        if (!$this->ManualCard->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ManualCard->Visible = false; // Disable update for API request
            } else {
                $this->ManualCard->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->ManualCash->CurrentValue = $this->ManualCash->FormValue;
        $this->ManualCard->CurrentValue = $this->ManualCard->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['ManualCash'] = $this->ManualCash->CurrentValue;
        $row['ManualCard'] = $this->ManualCard->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->DepositToBank->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->DepositToBank->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            if ($this->TransferTo->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->TransferTo->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            $this->ManualCash->EditValue = HtmlEncode($this->ManualCash->CurrentValue);
            ...
            $this->ManualCard->EditValue = HtmlEncode($this->ManualCard->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->ManualCash->Required) {
            if (!$this->ManualCash->IsDetailKey && EmptyValue($this->ManualCash->FormValue)) {
                $this->ManualCash->addErrorMessage(str_replace("%s", $this->ManualCash->caption(), $this->ManualCash->RequiredErrorMessage));
            }
        }
        if (!CheckNumber($this->ManualCash->FormValue)) {
            $this->ManualCash->addErrorMessage($this->ManualCash->getErrorMessage(false));
        }
        if ($this->ManualCard->Required) {
            if (!$this->ManualCard->IsDetailKey && EmptyValue($this->ManualCard->FormValue)) {
                $this->ManualCard->addErrorMessage(str_replace("%s", $this->ManualCard->caption(), $this->ManualCard->RequiredErrorMessage));
            }
        }
        if (!CheckNumber($this->ManualCard->FormValue)) {
            $this->ManualCard->addErrorMessage($this->ManualCard->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // TotalCash
        $this->TotalCash->setDbValueDef($rsnew, $this->TotalCash->CurrentValue, null, strval($this->TotalCash->CurrentValue) == "");

        // Cheque
        $this->Cheque->setDbValueDef($rsnew, $this->Cheque->CurrentValue, null, false);

        // Card
        $this->Card->setDbValueDef($rsnew, $this->Card->CurrentValue, null, strval($this->Card->CurrentValue) == "");
        ...
        // ModifiedUserName
        $this->ModifiedUserName->CurrentValue = CurrentUserName();
        ...
        // ManualCash
        $this->ManualCash->setDbValueDef($rsnew, $this->ManualCash->CurrentValue, null, strval($this->ManualCash->CurrentValue) == "");

        // ManualCard
        $this->ManualCard->setDbValueDef($rsnew, $this->ManualCard->CurrentValue, null, strval($this->ManualCard->CurrentValue) == "");
