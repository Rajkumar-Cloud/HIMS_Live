 ...
 * Page class
 */
class ViewBillingVoucherList extends ViewBillingVoucher
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewBillingVoucherList";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "ViewBillingVoucherAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ViewBillingVoucherDelete";
        $this->MultiUpdateUrl = "ViewBillingVoucherUpdate";
    ...
    public function resetFormError()
    {
        ...
        $this->DiscountAmount->clearErrorMessage();
        $this->BankName->clearErrorMessage();
        $this->_UserName->clearErrorMessage();
        $this->BillType->clearErrorMessage();
        $this->CancelBill->clearErrorMessage();
        $this->LabTest->clearErrorMessage();
        $this->sid->clearErrorMessage();
        $this->SidNo->clearErrorMessage();
        $this->createdDatettime->clearErrorMessage();
        $this->GoogleReviewID->clearErrorMessage();
        $this->CardType->clearErrorMessage();
        $this->PharmacyBill->clearErrorMessage();
        $this->cash->clearErrorMessage();
        $this->Card->clearErrorMessage();
    }
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'view_patient_services')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("ViewPatientServicesList?" . Config("TABLE_SHOW_MASTER") . "=view_billing_voucher&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewPatientServicesGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_billing_voucher')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'view_billing_voucher')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'view_billing_voucher') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages && $this->DetailPages["view_patient_services"] && $this->DetailPages["view_patient_services"]->Visible) {
            ...
            $url = "ViewPatientServicesPreview?t=view_billing_voucher&f=" . Encrypt($sqlwrk);
            ...
            if ($Security->allowList(CurrentProjectID() . 'view_billing_voucher')) {
                ...
                $links .= $link;
                $detaillnk = JsEncodeAttribute("ViewPatientServicesList?" . Config("TABLE_SHOW_MASTER") . "=view_billing_voucher&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_billing_voucher')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_patient_services");
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'view_billing_voucher')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=view_patient_services");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</button>";
            }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_DiscountAmount")) {
        ...
        if ($CurrentForm->hasValue("o_CardType")) {
        ...
        if ($CurrentForm->hasValue("o_PharmacyBill")) {
        ...
        if ($CurrentForm->hasValue("o_cash")) {
        ...
        if ($CurrentForm->hasValue("o_Card")) {
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['DiscountAmount']); // DiscountAmount
        $hash .= GetFieldHash($row['BankName']); // BankName
        $hash .= GetFieldHash($row['UserName']); // UserName
        $hash .= GetFieldHash($row['BillType']); // BillType
        $hash .= GetFieldHash($row['CancelBill']); // CancelBill
        $hash .= GetFieldHash($row['LabTest']); // LabTest
        $hash .= GetFieldHash($row['GoogleReviewID']); // GoogleReviewID
        $hash .= GetFieldHash($row['CardType']); // CardType
        $hash .= GetFieldHash($row['PharmacyBill']); // PharmacyBill
        $hash .= GetFieldHash($row['cash']); // cash
        $hash .= GetFieldHash($row['Card']); // Card
        return md5($hash);
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"ViewBillingVoucherSearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"view_billing_voucher\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"#\" onclick=\"return ew.modalDialogShow({lnk:this,btn:'SearchBtn',url:'ViewBillingVoucherSearch'});\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
