 ...
 * Page class
 */
class CrmLeadsRelationEdit extends CrmLeadsRelation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CrmLeadsRelationEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("leads_relationid") ?? Key(0) ?? Route(2)) !== null) {
                $this->leads_relationid->setQueryStringValue($keyValue);
                $this->leads_relationid->setOldValue($this->leads_relationid->QueryStringValue);
            } elseif (Post("leads_relationid") !== null) {
                $this->leads_relationid->setFormValue(Post("leads_relationid"));
                $this->leads_relationid->setOldValue($this->leads_relationid->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$loaded) { // Load record based on key
                    ...
                    $this->terminate("CrmLeadsRelationList"); // No matching record, return to list
                    return;
                }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "CrmLeadsRelationList") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->leads_relationid->Required) {
            if (!$this->leads_relationid->IsDetailKey && EmptyValue($this->leads_relationid->FormValue)) {
                $this->leads_relationid->addErrorMessage(str_replace("%s", $this->leads_relationid->caption(), $this->leads_relationid->RequiredErrorMessage));
            }
        }
