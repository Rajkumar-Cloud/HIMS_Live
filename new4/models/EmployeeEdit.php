...
class EmployeeEdit extends Employee
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("employee_address", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeTelephoneGrid");
        if (in_array("employee_telephone", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeEmailGrid");
        if (in_array("employee_email", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeHasDegreeGrid");
        if (in_array("employee_has_degree", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeHasExperienceGrid");
        if (in_array("employee_has_experience", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
        $detailPage = Container("EmployeeDocumentGrid");
        if (in_array("employee_document", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // driving_license
            $this->driving_license->setDbValueDef($rsnew, $this->driving_license->CurrentValue, null, $this->driving_license->ReadOnly);

            // driving_license_exp_date
            $this->driving_license_exp_date->setDbValueDef($rsnew, UnFormatDateTime($this->driving_license_exp_date->CurrentValue, 0), null, $this->driving_license_exp_date->ReadOnly);
            ...
            // pay_grade
            $this->pay_grade->setDbValueDef($rsnew, $this->pay_grade->CurrentValue, null, $this->pay_grade->ReadOnly);

            // work_station_id
            $this->work_station_id->setDbValueDef($rsnew, $this->work_station_id->CurrentValue, null, $this->work_station_id->ReadOnly);
            ...
            // private_email
            $this->private_email->setDbValueDef($rsnew, $this->private_email->CurrentValue, null, $this->private_email->ReadOnly);
            ...
            // indirect_supervisors
            $this->indirect_supervisors->setDbValueDef($rsnew, $this->indirect_supervisors->CurrentValue, null, $this->indirect_supervisors->ReadOnly);
            ...
            // custom5
            $this->custom5->setDbValueDef($rsnew, $this->custom5->CurrentValue, null, $this->custom5->ReadOnly);

            // custom6
            $this->custom6->setDbValueDef($rsnew, $this->custom6->CurrentValue, null, $this->custom6->ReadOnly);

            // custom7
            $this->custom7->setDbValueDef($rsnew, $this->custom7->CurrentValue, null, $this->custom7->ReadOnly);

            // custom8
            $this->custom8->setDbValueDef($rsnew, $this->custom8->CurrentValue, null, $this->custom8->ReadOnly);

            // custom9
            $this->custom9->setDbValueDef($rsnew, $this->custom9->CurrentValue, null, $this->custom9->ReadOnly);

            // custom10
            $this->custom10->setDbValueDef($rsnew, $this->custom10->CurrentValue, null, $this->custom10->ReadOnly);
            ...
            // ethnicity
            $this->ethnicity->setDbValueDef($rsnew, $this->ethnicity->CurrentValue, null, $this->ethnicity->ReadOnly);

            // immigration_status
            $this->immigration_status->setDbValueDef($rsnew, $this->immigration_status->CurrentValue, null, $this->immigration_status->ReadOnly);

            // approver1
            $this->approver1->setDbValueDef($rsnew, $this->approver1->CurrentValue, null, $this->approver1->ReadOnly);

            // approver2
            $this->approver2->setDbValueDef($rsnew, $this->approver2->CurrentValue, null, $this->approver2->ReadOnly);

            // approver3
            $this->approver3->setDbValueDef($rsnew, $this->approver3->CurrentValue, null, $this->approver3->ReadOnly);
