 ...
 * Page class
 */
class PharmacyPaymentList extends PharmacyPayment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PharmacyPaymentList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpharmacy_paymentlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PharmacyPaymentAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PharmacyPaymentDelete";
        $this->MultiUpdateUrl = "PharmacyPaymentUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fpharmacy_paymentlistsrch";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fpharmacy_paymentlistsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->PAYNO->AdvancedSearch->toJson(), ","); // Field PAYNO
        ...
        $filterList = Concat($filterList, $this->AccountNumber->AdvancedSearch->toJson(), ","); // Field AccountNumber
        $filterList = Concat($filterList, $this->chequeNumber->AdvancedSearch->toJson(), ","); // Field chequeNumber
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpharmacy_paymentlistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field PAYNO
        $this->PAYNO->AdvancedSearch->SearchValue = @$filter["x_PAYNO"];
        $this->PAYNO->AdvancedSearch->SearchOperator = @$filter["z_PAYNO"];
        $this->PAYNO->AdvancedSearch->SearchCondition = @$filter["v_PAYNO"];
        $this->PAYNO->AdvancedSearch->SearchValue2 = @$filter["y_PAYNO"];
        $this->PAYNO->AdvancedSearch->SearchOperator2 = @$filter["w_PAYNO"];
        $this->PAYNO->AdvancedSearch->save();
        ...
        // Field AccountNumber
        $this->AccountNumber->AdvancedSearch->SearchValue = @$filter["x_AccountNumber"];
        $this->AccountNumber->AdvancedSearch->SearchOperator = @$filter["z_AccountNumber"];
        $this->AccountNumber->AdvancedSearch->SearchCondition = @$filter["v_AccountNumber"];
        $this->AccountNumber->AdvancedSearch->SearchValue2 = @$filter["y_AccountNumber"];
        $this->AccountNumber->AdvancedSearch->SearchOperator2 = @$filter["w_AccountNumber"];
        $this->AccountNumber->AdvancedSearch->save();

        // Field chequeNumber
        $this->chequeNumber->AdvancedSearch->SearchValue = @$filter["x_chequeNumber"];
        $this->chequeNumber->AdvancedSearch->SearchOperator = @$filter["z_chequeNumber"];
        $this->chequeNumber->AdvancedSearch->SearchCondition = @$filter["v_chequeNumber"];
        $this->chequeNumber->AdvancedSearch->SearchValue2 = @$filter["y_chequeNumber"];
        $this->chequeNumber->AdvancedSearch->SearchOperator2 = @$filter["w_chequeNumber"];
        $this->chequeNumber->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->PAYNO, $arKeywords, $type);
        ...
        $this->buildBasicSearchSql($where, $this->AccountNumber, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->chequeNumber, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->PharmacyID); // PharmacyID
            ...
            $this->updateSort($this->TransPort); // TransPort
            $this->updateSort($this->AnyOther); // AnyOther
            ...
            $this->updateSort($this->AccountNumber); // AccountNumber
            $this->updateSort($this->chequeNumber); // chequeNumber
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->AccountNumber->setSort("");
                $this->chequeNumber->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_pharmacy_grn"
        $item = &$this->ListOptions->add("detail_pharmacy_grn");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'pharmacy_grn') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_pharmacy_grn"
        $opt = $this->ListOptions["detail_pharmacy_grn"];
        if ($Security->allowList(CurrentProjectID() . 'pharmacy_grn')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("pharmacy_grn", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("PharmacyGrnList?" . Config("TABLE_SHOW_MASTER") . "=pharmacy_payment&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PharmacyGrnGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "pharmacy_grn";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "pharmacy_grn";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_pharmacy_grn");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
                $detailPage = Container("PharmacyGrnGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'pharmacy_payment') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "pharmacy_grn";
                }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpharmacy_paymentlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpharmacy_paymentlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fpharmacy_paymentlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_pharmacy_grn"
        if ($this->DetailPages && $this->DetailPages["pharmacy_grn"] && $this->DetailPages["pharmacy_grn"]->Visible) {
            $link = "";
            $option = $this->ListOptions["detail_pharmacy_grn"];
            $url = "PharmacyGrnPreview?t=pharmacy_payment&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"pharmacy_grn\" data-url=\"" . $url . "\" class=\"btn-group btn-group-sm\">";
            if ($Security->allowList(CurrentProjectID() . 'pharmacy_payment')) {
                $label = $Language->TablePhrase("pharmacy_grn", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"pharmacy_grn\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("PharmacyGrnList?" . Config("TABLE_SHOW_MASTER") . "=pharmacy_payment&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . $Language->TablePhrase("pharmacy_grn", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</button>";
            }
            $detailPageObj = Container("PharmacyGrnGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</button>";
            }
            if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'pharmacy_payment')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=pharmacy_grn");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</button>";
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fpharmacy_paymentlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fpharmacy_paymentlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fpharmacy_paymentlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_pharmacy_payment" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_pharmacy_payment\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fpharmacy_paymentlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpharmacy_paymentlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
