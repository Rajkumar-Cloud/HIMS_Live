 ...
 * Page class
 */
class PharmacyBillingReturnList extends PharmacyBillingReturn
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PharmacyBillingReturnList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpharmacy_billing_returnlist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PharmacyBillingReturnAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PharmacyBillingReturnDelete";
        $this->MultiUpdateUrl = "PharmacyBillingReturnUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fpharmacy_billing_returnlistsrch";
    ...
    public function resetFormError()
    {
        ...
        $this->RealizationAmount->clearErrorMessage();
        ...
        $this->BillStatus->clearErrorMessage();
        $this->ReportHeader->clearErrorMessage();
        $this->PharID->clearErrorMessage();
    }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fpharmacy_billing_returnlistsrch");
        }
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpharmacy_billing_returnlistsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_view_pharmacy_pharled_return"
        $item = &$this->ListOptions->add("detail_view_pharmacy_pharled_return");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'view_pharmacy_pharled_return') && !$this->ShowMultipleDetails;
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("view_pharmacy_pharled_return");
    ...
    public function renderListOptions()
    {
        ...
        // "detail_view_pharmacy_pharled_return"
        $opt = $this->ListOptions["detail_view_pharmacy_pharled_return"];
        if ($Security->allowList(CurrentProjectID() . 'view_pharmacy_pharled_return')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("view_pharmacy_pharled_return", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("ViewPharmacyPharledReturnList?" . Config("TABLE_SHOW_MASTER") . "=pharmacy_billing_return&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewPharmacyPharledReturnGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pharmacy_billing_return')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "view_pharmacy_pharled_return";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'pharmacy_billing_return')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . HtmlImageAndText($caption) . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "view_pharmacy_pharled_return";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'pharmacy_billing_return')) {
                $caption = $Language->phrase("MasterDetailCopyLink");
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
                ...
                $detailCopyTblVar .= "view_pharmacy_pharled_return";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_view_pharmacy_pharled_return");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
                $detailPage = Container("ViewPharmacyPharledReturnGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'pharmacy_billing_return') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "view_pharmacy_pharled_return";
                }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpharmacy_billing_returnlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpharmacy_billing_returnlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fpharmacy_billing_returnlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function renderListOptionsExt()
    {
        ...
        $links = "";
        $btngrps = "";
        $sqlwrk = "`sretid`=" . AdjustSql($this->id->CurrentValue, $this->Dbid) . "";

        // Column "detail_view_pharmacy_pharled_return"
        if ($this->DetailPages && $this->DetailPages["view_pharmacy_pharled_return"] && $this->DetailPages["view_pharmacy_pharled_return"]->Visible) {
            $link = "";
            $option = $this->ListOptions["detail_view_pharmacy_pharled_return"];
            $url = "ViewPharmacyPharledReturnPreview?t=pharmacy_billing_return&f=" . Encrypt($sqlwrk);
            $btngrp = "<div data-table=\"view_pharmacy_pharled_return\" data-url=\"" . $url . "\" class=\"btn-group btn-group-sm\">";
            if ($Security->allowList(CurrentProjectID() . 'pharmacy_billing_return')) {
                $label = $Language->TablePhrase("view_pharmacy_pharled_return", "TblCaption");
                $link = "<li class=\"nav-item\"><a href=\"#\" class=\"nav-link\" data-toggle=\"tab\" data-table=\"view_pharmacy_pharled_return\" data-url=\"" . $url . "\">" . $label . "</a></li>";
                $links .= $link;
                $detaillnk = JsEncodeAttribute("ViewPharmacyPharledReturnList?" . Config("TABLE_SHOW_MASTER") . "=pharmacy_billing_return&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . $Language->TablePhrase("view_pharmacy_pharled_return", "TblCaption") . "\" onclick=\"window.location='" . $detaillnk . "';return false;\">" . $Language->phrase("MasterDetailListLink") . "</button>";
            }
            $detailPageObj = Container("ViewPharmacyPharledReturnGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pharmacy_billing_return')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'pharmacy_billing_return')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=view_pharmacy_pharled_return");
                $btngrp .= "<button type=\"button\" class=\"btn btn-default\" title=\"" . HtmlTitle($caption) . "\" onclick=\"window.location='" . HtmlEncode($url) . "';return false;\">" . $caption . "</button>";
            }
    ...
    protected function loadSearchValues()
    {
        ...
        // PharID
        if (!$this->isAddOrEdit() && $this->PharID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->PharID->AdvancedSearch->SearchValue != "" || $this->PharID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['mrnno']); // mrnno
        $hash .= GetFieldHash($row['PatientName']); // PatientName
        $hash .= GetFieldHash($row['Mobile']); // Mobile
        $hash .= GetFieldHash($row['IP_OP']); // IP_OP
        $hash .= GetFieldHash($row['Doctor']); // Doctor
        $hash .= GetFieldHash($row['voucher_type']); // voucher_type
        $hash .= GetFieldHash($row['ModeofPayment']); // ModeofPayment
        $hash .= GetFieldHash($row['Amount']); // Amount
        $hash .= GetFieldHash($row['AnyDues']); // AnyDues
        ...
        $hash .= GetFieldHash($row['RealizationAmount']); // RealizationAmount
        $hash .= GetFieldHash($row['CId']); // CId
        $hash .= GetFieldHash($row['PartnerName']); // PartnerName
        $hash .= GetFieldHash($row['BillNumber']); // BillNumber
        $hash .= GetFieldHash($row['CardNumber']); // CardNumber
        $hash .= GetFieldHash($row['BillStatus']); // BillStatus
        $hash .= GetFieldHash($row['ReportHeader']); // ReportHeader
        $hash .= GetFieldHash($row['PharID']); // PharID
        return md5($hash);
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fpharmacy_billing_returnlist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fpharmacy_billing_returnlist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fpharmacy_billing_returnlist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_pharmacy_billing_return" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_pharmacy_billing_return\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fpharmacy_billing_returnlist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpharmacy_billing_returnlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
