 ...
 * Page class
 */
class ViewLabServicessView extends ViewLabServicess
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewLabServicessView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("id") ?? Route("id")) !== null) {
                ...
                $returnUrl = "ViewLabServicessList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "ViewLabServicessList"; // No matching record, return to list
                    }
                    break;
            }
            ...
            $returnUrl = "ViewLabServicessList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ViewLabResultreleaseddList?" . Config("TABLE_SHOW_MASTER") . "=view_lab_servicess&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_lab_servicess')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_lab_resultreleasedd"))) . "\">" . HtmlImageAndText($Language->phrase("MasterDetailViewLink")) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("view_lab_resultreleasedd", explode(",", $this->getCurrentDetailTable()))) {
            $view_lab_resultreleasedd = Container("view_lab_resultreleasedd");
            ...
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $view_lab_resultreleasedd->exportDocument($doc, new Recordset($rsdetail), 1, $detailcnt);
                    $doc->Table = $oldtbl;
                }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ViewLabServicessList"), "", $this->TableVar, true);
        $pageId = "view";
