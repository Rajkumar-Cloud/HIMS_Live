 ...
 * Page class
 */
class PresSideeffectTableAdd extends PresSideeffectTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PresSideeffectTableAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PresSideeffectTableList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PresSideeffectTableList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PresSideeffectTableView") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->SideEffects->Required) {
            if (!$this->SideEffects->IsDetailKey && EmptyValue($this->SideEffects->FormValue)) {
                $this->SideEffects->addErrorMessage(str_replace("%s", $this->SideEffects->caption(), $this->SideEffects->RequiredErrorMessage));
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PresSideeffectTableList"), "", $this->TableVar, true);
