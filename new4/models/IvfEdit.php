...
class IvfEdit extends Ivf
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->Male->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->Male->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->Male->Lookup->renderViewRow($rswrk[0]);
    ...
    protected function validateForm()
    {
        ...
        if ($this->malepic->Required) {
            if (!$this->malepic->IsDetailKey && EmptyValue($this->malepic->FormValue)) {
                $this->malepic->addErrorMessage(str_replace("%s", $this->malepic->caption(), $this->malepic->RequiredErrorMessage));
            }
        }
        ...
        if ($this->Mgendar->Required) {
            if (!$this->Mgendar->IsDetailKey && EmptyValue($this->Mgendar->FormValue)) {
                $this->Mgendar->addErrorMessage(str_replace("%s", $this->Mgendar->caption(), $this->Mgendar->RequiredErrorMessage));
            }
        }
        if ($this->Fgendar->Required) {
            if (!$this->Fgendar->IsDetailKey && EmptyValue($this->Fgendar->FormValue)) {
                $this->Fgendar->addErrorMessage(str_replace("%s", $this->Fgendar->caption(), $this->Fgendar->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("ivf_treatment_plan", $detailTblVar) && $detailPage->DetailEdit) {
    ...
    protected function editRow()
    {
        ...
        if ($this->Female->CurrentValue != "") { // Check field with unique index
            $filterChk = "(`Female` = " . AdjustSql($this->Female->CurrentValue, $this->Dbid) . ")";
        ...
        if (!$rsold) {
            ...
            // Male
            $this->Male->setDbValueDef($rsnew, $this->Male->CurrentValue, 0, $this->Male->ReadOnly);
            ...
            // modifieddatetime
            $this->modifieddatetime->CurrentValue = CurrentDate();
            ...
            // malepic
            $this->malepic->setDbValueDef($rsnew, $this->malepic->CurrentValue, "", $this->malepic->ReadOnly);

            // femalepic
            $this->femalepic->setDbValueDef($rsnew, $this->femalepic->CurrentValue, "", $this->femalepic->ReadOnly);

            // Mgendar
            $this->Mgendar->setDbValueDef($rsnew, $this->Mgendar->CurrentValue, "", $this->Mgendar->ReadOnly);

            // Fgendar
            $this->Fgendar->setDbValueDef($rsnew, $this->Fgendar->CurrentValue, "", $this->Fgendar->ReadOnly);
