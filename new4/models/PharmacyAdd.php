...
class PharmacyAdd extends Pharmacy
{
    ...
    // Page object name
    public $PageObjName = "PharmacyAdd";
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->ip_id->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->ip_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                foreach ($arwrk as &$row)
                    $row = $this->ip_id->Lookup->renderViewRow($row);
    ...
    protected function validateForm()
    {
        ...
        if (in_array("pharmacy_services", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("PharmacyServicesGrid");
            if (in_array("pharmacy_services", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->pharmacy_id->setSessionValue($this->id->CurrentValue); // Set master key
                ...
                if (!$addRow) {
                $detailPage->pharmacy_id->setSessionValue(""); // Clear master key if insert failed
