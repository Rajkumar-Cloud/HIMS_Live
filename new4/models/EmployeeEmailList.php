 ...
 * Page class
 */
class EmployeeEmailList extends EmployeeEmail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EmployeeEmailList";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "EmployeeEmailAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "EmployeeEmailDelete";
        $this->MultiUpdateUrl = "EmployeeEmailUpdate";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("employee_email"));
                $doc->Text = @$content;
    ...
    public function resetFormError()
    {
        ...
        $this->email_type->clearErrorMessage();
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->email_type->ViewValue === null) { // Lookup from database
                    ...
                    $sqlWrk = $this->email_type->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->email_type->Lookup->renderViewRow($rswrk[0]);
            ...
            if ($this->email_type->ViewValue !== null) { // Load from cache
                ...
                $sqlWrk = $this->email_type->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->email_type->EditValue = $arwrk;
            }
            $this->email_type->PlaceHolder = RemoveHtml($this->email_type->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->email_type->Required) {
            if (!$this->email_type->IsDetailKey && EmptyValue($this->email_type->FormValue)) {
                $this->email_type->addErrorMessage(str_replace("%s", $this->email_type->caption(), $this->email_type->RequiredErrorMessage));
            }
        }
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash .= GetFieldHash($row['email_type']); // email_type
