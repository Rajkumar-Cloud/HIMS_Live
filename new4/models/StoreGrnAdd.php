 ...
 * Page class
 */
class StoreGrnAdd extends StoreGrn
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StoreGrnAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("StoreGrnList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "StoreGrnList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "StoreGrnView") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->BRCODE->ViewValue !== null) { // Load from cache
                ...
                $lookupFilter = $lookupFilter->bindTo($this);
                $sqlWrk = $this->BRCODE->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
    ...
    protected function validateForm()
    {
        ...
        if (in_array("view_store_grn", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
                        ...
                        if (file_exists($tempPath . $file)) {
                            ...
                            if ($file1 != $file) { // Rename temp file
                                while (file_exists($tempPath . $file1) || file_exists($this->BillUpload->physicalUploadPath() . $file1)) { // Make sure no file name clash
                                    $file1 = UniqueFilename([$this->BillUpload->physicalUploadPath(), $tempPath], $file1, true); // Use indexed name
                                }
        ...
        if ($addRow) {
            ...
            $detailPage = Container("ViewStoreGrnGrid");
            if (in_array("view_store_grn", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->grnid->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "view_store_grn"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->grnid->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
