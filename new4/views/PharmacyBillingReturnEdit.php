...
loadjs.ready("head", function () {
    ...
    fpharmacy_billing_returnedit.lists.Reception = <?= $Page->Reception->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.ModeofPayment = <?= $Page->ModeofPayment->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.RIDNO = <?= $Page->RIDNO->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.CId = <?= $Page->CId->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.PatId = <?= $Page->PatId->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.RefferedBy = <?= $Page->RefferedBy->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.DrID = <?= $Page->DrID->toClientList($Page) ?>;
    fpharmacy_billing_returnedit.lists.ReportHeader = <?= $Page->ReportHeader->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpharmacy_billing_returnedit" id="fpharmacy_billing_returnedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pharmacy_billing_return_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_pharmacy_billing_return_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
...
<template id="tpm_pharmacy_billing_returnedit">
<div id="ct_PharmacyBillingReturnEdit"><style>
...
<?php
    if (in_array("view_pharmacy_pharled_return", explode(",", $Page->getCurrentDetailTable())) && $view_pharmacy_pharled_return->DetailEdit) {
?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_pharmacy_billing_returnedit", "tpm_pharmacy_billing_returnedit", "pharmacy_billing_returnedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
