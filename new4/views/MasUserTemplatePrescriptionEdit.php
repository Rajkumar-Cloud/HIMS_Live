...
loadjs.ready("head", function () {
    ...
    fmas_user_template_prescriptionedit.lists.TemplateName = <?= $Page->TemplateName->toClientList($Page) ?>;
    fmas_user_template_prescriptionedit.lists.Medicine = <?= $Page->Medicine->toClientList($Page) ?>;
    fmas_user_template_prescriptionedit.lists.PreRoute = <?= $Page->PreRoute->toClientList($Page) ?>;
    fmas_user_template_prescriptionedit.lists.TimeOfTaking = <?= $Page->TimeOfTaking->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fmas_user_template_prescriptionedit" id="fmas_user_template_prescriptionedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fmas_user_template_prescriptionedit"], function() {
    fmas_user_template_prescriptionedit.createAutoSuggest(Object.assign({"id":"x_Medicine","forceSelect":true}, ew.vars.tables.mas_user_template_prescription.fields.Medicine.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["fmas_user_template_prescriptionedit"], function() {
    fmas_user_template_prescriptionedit.createAutoSuggest(Object.assign({"id":"x_PreRoute","forceSelect":false}, ew.vars.tables.mas_user_template_prescription.fields.PreRoute.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["fmas_user_template_prescriptionedit"], function() {
    fmas_user_template_prescriptionedit.createAutoSuggest(Object.assign({"id":"x_TimeOfTaking","forceSelect":false}, ew.vars.tables.mas_user_template_prescription.fields.TimeOfTaking.autoSuggestOptions));
});
</script>
...
<span id="el_mas_user_template_prescription_Status">
    <select
        ...
        data-select2-id="mas_user_template_prescription_x_Status"
        data-table="mas_user_template_prescription"
        data-field="x_Status"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='mas_user_template_prescription_x_Status']"),
        options = { name: "x_Status", selectId: "mas_user_template_prescription_x_Status", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.mas_user_template_prescription.fields.Status.selectOptions);
    ew.createSelect(options);
});
</script>
</span>
