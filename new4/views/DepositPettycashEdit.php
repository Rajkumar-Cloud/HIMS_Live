...
// Page object
$DepositPettycashEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fdeposit_pettycashedit;
loadjs.ready("head", function () {
    ...
    fdeposit_pettycashedit.addFields([
    ...
    // Form_CustomValidate
    fdeposit_pettycashedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdeposit_pettycashedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fdeposit_pettycashedit.lists.TransType = <?= $Page->TransType->toClientList($Page) ?>;
    fdeposit_pettycashedit.lists.TerminalNumber = <?= $Page->TerminalNumber->toClientList($Page) ?>;
    fdeposit_pettycashedit.lists.AccoundHead = <?= $Page->AccoundHead->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fdeposit_pettycashedit" id="fdeposit_pettycashedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_deposit_pettycash_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_deposit_pettycash_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="deposit_pettycash" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
