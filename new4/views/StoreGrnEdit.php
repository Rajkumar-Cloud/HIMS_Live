...
loadjs.ready("head", function () {
    ...
    fstore_grnedit = currentForm = new ew.Form("fstore_grnedit", "edit");
    ...
    // Form_CustomValidate
    fstore_grnedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fstore_grnedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fstore_grnedit.lists.PC = <?= $Page->PC->toClientList($Page) ?>;
    fstore_grnedit.lists.BRCODE = <?= $Page->BRCODE->toClientList($Page) ?>;
    loadjs.done("fstore_grnedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fstore_grnedit" id="fstore_grnedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_store_grn_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_store_grn_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<template id="tpx_store_grn_id"><span id="el_store_grn_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span></template>
<input type="hidden" data-table="store_grn" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fstore_grnedit", "datetimepicker"], function() {
...
<template id="tpm_store_grnedit">
<div id="ct_StoreGrnEdit"><style>
...
<?php
    if (in_array("view_store_grn", explode(",", $Page->getCurrentDetailTable())) && $view_store_grn->DetailEdit) {
?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_store_grnedit", "tpm_store_grnedit", "store_grnedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
