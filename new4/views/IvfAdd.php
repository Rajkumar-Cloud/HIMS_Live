...
loadjs.ready("head", function () {
    ...
    fivfadd.lists.Male = <?= $Page->Male->toClientList($Page) ?>;
    fivfadd.lists.Female = <?= $Page->Female->toClientList($Page) ?>;
    fivfadd.lists.status = <?= $Page->status->toClientList($Page) ?>;
    fivfadd.lists.ReferedBy = <?= $Page->ReferedBy->toClientList($Page) ?>;
    fivfadd.lists.ARTCYCLE = <?= $Page->ARTCYCLE->toClientList($Page) ?>;
    fivfadd.lists.RESULT = <?= $Page->RESULT->toClientList($Page) ?>;
    fivfadd.lists.DrID = <?= $Page->DrID->toClientList($Page) ?>;
    loadjs.done("fivfadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fivfadd" id="fivfadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<template id="tpm_ivfadd">
<div id="ct_IvfAdd"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_ivfadd", "tpm_ivfadd", "ivfadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
