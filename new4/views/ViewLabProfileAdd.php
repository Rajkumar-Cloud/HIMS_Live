...
// Page object
$ViewLabProfileAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_lab_profileadd;
loadjs.ready("head", function () {
    ...
    fview_lab_profileadd.addFields([
    ...
    // Form_CustomValidate
    fview_lab_profileadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_lab_profileadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_lab_profileadd.lists.SERVICE_TYPE = <?= $Page->SERVICE_TYPE->toClientList($Page) ?>;
    fview_lab_profileadd.lists.DEPARTMENT = <?= $Page->DEPARTMENT->toClientList($Page) ?>;
    fview_lab_profileadd.lists.TestType = <?= $Page->TestType->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_lab_profileadd" id="fview_lab_profileadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fview_lab_profileadd", "editor"], function() {
	ew.createEditor("fview_lab_profileadd", "x_Interpretation", 35, 4, <?= $Page->Interpretation->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<template id="tpm_view_lab_profileadd">
<div id="ct_ViewLabProfileAdd"><style>
...
<?php
    if (in_array("lab_profile_details", explode(",", $Page->getCurrentDetailTable())) && $lab_profile_details->DetailAdd) {
?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_lab_profileadd", "tpm_view_lab_profileadd", "view_lab_profileadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
