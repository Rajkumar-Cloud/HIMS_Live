...
// Page object
$PatientFinalDiagnosisAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpatient_final_diagnosisadd;
loadjs.ready("head", function () {
    ...
    fpatient_final_diagnosisadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fpatient_final_diagnosisadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpatient_final_diagnosisadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpatient_final_diagnosisadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpatient_final_diagnosisadd.lists.FinalDiagnosisTemplate = <?= $Page->FinalDiagnosisTemplate->toClientList($Page) ?>;
    fpatient_final_diagnosisadd.lists.TreatmentsTemplate = <?= $Page->TreatmentsTemplate->toClientList($Page) ?>;
    fpatient_final_diagnosisadd.lists.PatientSearch = <?= $Page->PatientSearch->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpatient_final_diagnosisadd" id="fpatient_final_diagnosisadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fpatient_final_diagnosisadd", "editor"], function() {
	ew.createEditor("fpatient_final_diagnosisadd", "x_FinalDiagnosis", 35, 25, <?= $Page->FinalDiagnosis->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<script>
loadjs.ready(["fpatient_final_diagnosisadd", "editor"], function() {
	ew.createEditor("fpatient_final_diagnosisadd", "x_Treatments", 35, 25, <?= $Page->Treatments->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<template id="tpm_patient_final_diagnosisadd">
<div id="ct_PatientFinalDiagnosisAdd"><style>
...
<slot class="ew-slot" name="tpc_patient_final_diagnosis_PatientSearch"></slot>&nbsp;<slot class="ew-slot" name="tpx_patient_final_diagnosis_PatientSearch"></slot>	
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_patient_final_diagnosisadd", "tpm_patient_final_diagnosisadd", "patient_final_diagnosisadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
