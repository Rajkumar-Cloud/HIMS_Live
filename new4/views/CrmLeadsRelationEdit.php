...
// Page object
$CrmLeadsRelationEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fcrm_leads_relationedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fcrm_leads_relationedit = currentForm = new ew.Form("fcrm_leads_relationedit", "edit");
    ...
    fcrm_leads_relationedit.addFields([
        ["leads_relationid", [fields.leads_relationid.visible && fields.leads_relationid.required ? ew.Validators.required(fields.leads_relationid.caption) : null], fields.leads_relationid.isInvalid],
    ...
    // Form_CustomValidate
    fcrm_leads_relationedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrm_leads_relationedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcrm_leads_relationedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcrm_leads_relationedit" id="fcrm_leads_relationedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->leads_relationid->Visible) { // leads_relationid ?>
    <div id="r_leads_relationid" class="form-group row">
        <label id="elh_crm_leads_relation_leads_relationid" class="<?= $Page->LeftColumnClass ?>"><?= $Page->leads_relationid->caption() ?><?= $Page->leads_relationid->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->leads_relationid->cellAttributes() ?>>
<span id="el_crm_leads_relation_leads_relationid">
<span<?= $Page->leads_relationid->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->leads_relationid->getDisplayValue($Page->leads_relationid->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="crm_leads_relation" data-field="x_leads_relationid" data-hidden="1" name="x_leads_relationid" id="x_leads_relationid" value="<?= HtmlEncode($Page->leads_relationid->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
