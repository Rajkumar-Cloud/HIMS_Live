...
// Page object
$MasPharmacyEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fmas_pharmacyedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "mas_pharmacy")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.mas_pharmacy)
        ew.vars.tables.mas_pharmacy = currentTable;
    fmas_pharmacyedit.addFields([
    ...
    // Form_CustomValidate
    fmas_pharmacyedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmas_pharmacyedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmas_pharmacyedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fmas_pharmacyedit" id="fmas_pharmacyedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_mas_pharmacy_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_mas_pharmacy_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="mas_pharmacy" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_mas_pharmacy_status">
    <select
        ...
        data-select2-id="mas_pharmacy_x_status"
        data-table="mas_pharmacy"
        data-field="x_status"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='mas_pharmacy_x_status']"),
        options = { name: "x_status", selectId: "mas_pharmacy_x_status", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
    options.dropdownParent = $(el).closest("#ew-modal-dialog, #ew-add-opt-dialog")[0];
    Object.assign(options, ew.vars.tables.mas_pharmacy.fields.status.selectOptions);
...
<?php
    if (in_array("pharmacy_services", explode(",", $Page->getCurrentDetailTable())) && $pharmacy_services->DetailEdit) {
?>
