...
$Page->showMessage();
?>
<form name="fhelpadd" id="fhelpadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fhelpadd", "editor"], function() {
	ew.createEditor("fhelpadd", "x_Description", 35, 400, <?= $Page->Description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_helpadd", "tpm_helpadd", "helpadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
