...
// Page object
$ViewIpAdvanceAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_ip_advanceadd;
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fview_ip_advanceadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_ip_advanceadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_ip_advanceadd.lists.ModeofPayment = <?= $Page->ModeofPayment->toClientList($Page) ?>;
    fview_ip_advanceadd.lists.Reception = <?= $Page->Reception->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_ip_advanceadd" id="fview_ip_advanceadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } else { ?>
<template id="tpx_view_ip_advance_PatID"><span id="el_view_ip_advance_PatID">
<input type="<?= $Page->PatID->getInputTextType() ?>" data-table="view_ip_advance" data-field="x_PatID" name="x_PatID" id="x_PatID" size="30" placeholder="<?= HtmlEncode($Page->PatID->getPlaceHolder()) ?>" value="<?= $Page->PatID->EditValue ?>"<?= $Page->PatID->editAttributes() ?> aria-describedby="x_PatID_help">
...
<input type="hidden" is="selection-list" data-table="view_ip_advance" data-field="x_Reception" data-type="text" data-multiple="0" data-lookup="1" data-value-separator="<?= $Page->Reception->displayValueSeparatorAttribute() ?>" name="x_Reception" id="x_Reception" value="<?= $Page->Reception->CurrentValue ?>"<?= $Page->Reception->editAttributes() ?>>
</span></template>
<?php } ?>
</div></div>
    </div>
...
<?php } else { ?>
<template id="tpx_view_ip_advance_mrnno"><span id="el_view_ip_advance_mrnno">
<input type="<?= $Page->mrnno->getInputTextType() ?>" data-table="view_ip_advance" data-field="x_mrnno" name="x_mrnno" id="x_mrnno" size="30" maxlength="45" placeholder="<?= HtmlEncode($Page->mrnno->getPlaceHolder()) ?>" value="<?= $Page->mrnno->EditValue ?>"<?= $Page->mrnno->editAttributes() ?> aria-describedby="x_mrnno_help">
...
<template id="tpm_view_ip_advanceadd">
<div id="ct_ViewIpAdvanceAdd"><style>
...
<div class="row">
	<div class="col-4">
		<h3 class="card-title"><slot class="ew-slot" name="tpc_view_ip_advance_Reception"></slot>&nbsp;<slot class="ew-slot" name="tpx_view_ip_advance_Reception"></slot></h3>
	</div>
		...
		<div hidden class="col-4">
		<h3 class="card-title"><slot class="ew-slot" name="tpc_view_ip_advance_mrnno"></slot>&nbsp;<slot class="ew-slot" name="tpx_view_ip_advance_mrnno"></slot></h3>
	</div>
</div>
<div class="row">
	<div class="col-12">
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_ip_advanceadd", "tpm_view_ip_advanceadd", "view_ip_advanceadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
