...
// Page object
$RecruitmentJobEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var frecruitment_jobedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    frecruitment_jobedit = currentForm = new ew.Form("frecruitment_jobedit", "edit");
    ...
    frecruitment_jobedit.addFields([
    ...
    // Form_CustomValidate
    frecruitment_jobedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frecruitment_jobedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frecruitment_jobedit.lists.showSalary = <?= $Page->showSalary->toClientList($Page) ?>;
    frecruitment_jobedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("frecruitment_jobedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="frecruitment_jobedit" id="frecruitment_jobedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_recruitment_job_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_recruitment_job_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="recruitment_job" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["frecruitment_jobedit", "datetimepicker"], function() {
