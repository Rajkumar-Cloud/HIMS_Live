...
loadjs.ready("head", function () {
    ...
    fpharmacy_poadd.lists.PC = <?= $Page->PC->toClientList($Page) ?>;
    fpharmacy_poadd.lists.BRCODE = <?= $Page->BRCODE->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpharmacy_poadd" id="fpharmacy_poadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<template id="tpm_pharmacy_poadd">
<div id="ct_PharmacyPoAdd"><style>
...
<?php
    if (in_array("pharmacy_purchaseorder", explode(",", $Page->getCurrentDetailTable())) && $pharmacy_purchaseorder->DetailAdd) {
?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_pharmacy_poadd", "tpm_pharmacy_poadd", "pharmacy_poadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
