...
// Page object
$ViewReportRevenueList = &$Page;
?>
<?php if (!$Page->isExport()) { ?>
<script>
var currentForm, currentPageID;
var fview_report_revenuelist;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "list";
    fview_report_revenuelist = currentForm = new ew.Form("fview_report_revenuelist", "list");
    fview_report_revenuelist.formKeyCountName = '<?= $Page->FormKeyCountName ?>';
    loadjs.done("fview_report_revenuelist");
});
var fview_report_revenuelistsrch, currentSearchForm, currentAdvancedSearchForm;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object for search
    fview_report_revenuelistsrch = currentSearchForm = new ew.Form("fview_report_revenuelistsrch");

    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "view_report_revenue")) ?>,
        fields = currentTable.fields;
    fview_report_revenuelistsrch.addFields([
    ...
    // Set invalid fields
    $(function() {
        fview_report_revenuelistsrch.setInvalid();
    });

    // Validate form
    fview_report_revenuelistsrch.validate = function () {
    ...
    // Form_CustomValidate
    fview_report_revenuelistsrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_report_revenuelistsrch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists

    // Filters
    fview_report_revenuelistsrch.filterList = <?= $Page->getFilterList() ?>;
    loadjs.done("fview_report_revenuelistsrch");
});
</script>
<style type="text/css">
...
<form name="fview_report_revenuelistsrch" id="fview_report_revenuelistsrch" class="form-inline ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>">
<div id="fview_report_revenuelistsrch-search-panel" class="<?= $Page->SearchPanelClass ?>">
...
<option value="BETWEEN"<?= $Page->Date->AdvancedSearch->SearchOperator == "BETWEEN" ? " selected" : "" ?>><?= $Language->phrase("BETWEEN") ?></option>
        ...
        <span id="el_view_report_revenue_Date" class="ew-search-field">
<input type="<?= $Page->Date->getInputTextType() ?>" data-table="view_report_revenue" data-field="x_Date" data-format="7" name="x_Date" id="x_Date" placeholder="<?= HtmlEncode($Page->Date->getPlaceHolder()) ?>" value="<?= $Page->Date->EditValue ?>"<?= $Page->Date->editAttributes() ?>>
...
<script>
loadjs.ready(["fview_report_revenuelistsrch", "datetimepicker"], function() {
...
<?php } ?>
        ...
        <span id="el2_view_report_revenue_Date" class="ew-search-field2 d-none">
<input type="<?= $Page->Date->getInputTextType() ?>" data-table="view_report_revenue" data-field="x_Date" data-format="7" name="y_Date" id="y_Date" placeholder="<?= HtmlEncode($Page->Date->getPlaceHolder()) ?>" value="<?= $Page->Date->EditValue2 ?>"<?= $Page->Date->editAttributes() ?>>
<div class="invalid-feedback"><?= $Page->Date->getErrorMessage(false) ?></div>
<?php if (!$Page->Date->ReadOnly && !$Page->Date->Disabled && !isset($Page->Date->EditAttrs["readonly"]) && !isset($Page->Date->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fview_report_revenuelistsrch", "datetimepicker"], function() {
...
<div class="card ew-card ew-grid<?php if ($Page->isAddOrEdit()) { ?> ew-grid-add-edit<?php } ?> view_report_revenue">
<?php if (!$Page->isExport()) { ?>
...
<?php } ?>
<form name="fview_report_revenuelist" id="fview_report_revenuelist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="view_report_revenue">
<div id="gmp_view_report_revenue" class="<?= ResponsiveTableClass() ?>card-body ew-grid-middle-panel">
...
$Page->ListOptions->render("header", "left", "", "block", $Page->TableVar, "view_report_revenuelist");
?>
<?php if ($Page->Date->Visible) { // Date ?>
        <th data-name="Date" class="<?= $Page->Date->headerCellClass() ?>"><div id="elh_view_report_revenue_Date" class="view_report_revenue_Date"><?= $Page->renderSort($Page->Date) ?></div></th>
<?php } ?>
<?php if ($Page->BillType->Visible) { // BillType ?>
        <th data-name="BillType" class="<?= $Page->BillType->headerCellClass() ?>"><div id="elh_view_report_revenue_BillType" class="view_report_revenue_BillType"><?= $Page->renderSort($Page->BillType) ?></div></th>
<?php } ?>
<?php if ($Page->TotalAmount->Visible) { // TotalAmount ?>
        <th data-name="TotalAmount" class="<?= $Page->TotalAmount->headerCellClass() ?>"><div id="elh_view_report_revenue_TotalAmount" class="view_report_revenue_TotalAmount"><?= $Page->renderSort($Page->TotalAmount) ?></div></th>
<?php } ?>
<?php if ($Page->HospID->Visible) { // HospID ?>
        <th data-name="HospID" class="<?= $Page->HospID->headerCellClass() ?>"><div id="elh_view_report_revenue_HospID" class="view_report_revenue_HospID"><?= $Page->renderSort($Page->HospID) ?></div></th>
<?php } ?>
<?php
// Render list options (header, right)
$Page->ListOptions->render("header", "right", "", "block", $Page->TableVar, "view_report_revenuelist");
?>
    </tr>
</thead>
<tbody>
...
while ($Page->RecordCount < $Page->StopRecord) {
    ...
    if ($Page->RecordCount >= $Page->StartRecord) {
        ...
        $Page->RowAttrs->merge(["data-rowindex" => $Page->RowCount, "id" => "r" . $Page->RowCount . "_view_report_revenue", "data-rowtype" => $Page->RowType]);
...
$Page->ListOptions->render("body", "left", $Page->RowCount, "block", $Page->TableVar, "view_report_revenuelist");
?>
    <?php if ($Page->Date->Visible) { // Date ?>
        <td data-name="Date" <?= $Page->Date->cellAttributes() ?>>
<template id="tpx<?= $Page->RowCount ?>_view_report_revenue_Date"><span id="el<?= $Page->RowCount ?>_view_report_revenue_Date">
...
<template id="tpx<?= $Page->RowCount ?>_view_report_revenue_BillType"><span id="el<?= $Page->RowCount ?>_view_report_revenue_BillType">
...
<template id="tpx<?= $Page->RowCount ?>_view_report_revenue_TotalAmount"><span id="el<?= $Page->RowCount ?>_view_report_revenue_TotalAmount">
...
<template id="tpx<?= $Page->RowCount ?>_view_report_revenue_HospID"><span id="el<?= $Page->RowCount ?>_view_report_revenue_HospID">
...
$Page->ListOptions->render("body", "right", $Page->RowCount, "block", $Page->TableVar, "view_report_revenuelist");
?>
    </tr>
<?php
    }
...
<template id="tpm_view_report_revenuelist">
<div id="ct_ViewReportRevenueList"><?php if ($Page->RowCount > 0) { ?>
...
<?php } ?>
  ...
  <?php if ($Page->TotalRecords > 0 && !$view_report_revenue->isGridAdd() && !$view_report_revenue->isGridEdit()) { ?>
<tfoot>
  </tfoot>
<?php } ?>  
</table>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_report_revenuelist", "tpm_view_report_revenuelist", "view_report_revenuelist", "<?= $Page->CustomExport ?>", ew.templateData);
...
loadjs.ready("head", function() {
    ew.addEventHandlers("view_report_revenue");
});
</script>
<script>
...
loadjs.ready("fixedheadertable", function () {
    ew.fixedHeaderTable({
        delay: 0,
        container: "gmp_view_report_revenue",
        width: "95%",
        height: ""
    });
});
