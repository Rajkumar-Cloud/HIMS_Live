...
loadjs.ready("head", function () {
    ...
    fuser_loginedit.lists.User_Name = <?= $Page->User_Name->toClientList($Page) ?>;
    fuser_loginedit.lists.email_verified = <?= $Page->email_verified->toClientList($Page) ?>;
    fuser_loginedit.lists._UserLevel = <?= $Page->_UserLevel->toClientList($Page) ?>;
    fuser_loginedit.lists.HospID = <?= $Page->HospID->toClientList($Page) ?>;
    fuser_loginedit.lists.PharID = <?= $Page->PharID->toClientList($Page) ?>;
    fuser_loginedit.lists.StoreID = <?= $Page->StoreID->toClientList($Page) ?>;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_user_login_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_user_login_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="user_login" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
loadjs.ready(["fuser_loginedit"], function() {
    fuser_loginedit.createAutoSuggest(Object.assign({"id":"x_User_Name","forceSelect":false}, ew.vars.tables.user_login.fields.User_Name.autoSuggestOptions));
});
</script>
...
<div class="input-group">
    <input type="password" name="x__password" id="x__password" autocomplete="new-password" data-field="x__password" value="<?= $Page->_password->EditValue ?>" size="30" maxlength="45" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
