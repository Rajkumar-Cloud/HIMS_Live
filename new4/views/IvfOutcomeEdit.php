...
loadjs.ready("head", function () {
    ...
    fivf_outcomeedit.lists.Gestation = <?= $Page->Gestation->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Urine = <?= $Page->Urine->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Miscarriage = <?= $Page->Miscarriage->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Induced1 = <?= $Page->Induced1->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Type = <?= $Page->Type->toClientList($Page) ?>;
    fivf_outcomeedit.lists.FoetalDeath = <?= $Page->FoetalDeath->toClientList($Page) ?>;
    fivf_outcomeedit.lists.FerinatalDeath = <?= $Page->FerinatalDeath->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Ectopic = <?= $Page->Ectopic->toClientList($Page) ?>;
    fivf_outcomeedit.lists.Extra = <?= $Page->Extra->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fivf_outcomeedit" id="fivf_outcomeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_ivf_outcome_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_ivf_outcome_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
...
<template id="tpm_ivf_outcomeedit">
<div id="ct_IvfOutcomeEdit"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_ivf_outcomeedit", "tpm_ivf_outcomeedit", "ivf_outcomeedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
