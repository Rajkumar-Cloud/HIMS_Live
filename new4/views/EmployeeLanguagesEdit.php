...
// Page object
$EmployeeLanguagesEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var femployee_languagesedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    femployee_languagesedit = currentForm = new ew.Form("femployee_languagesedit", "edit");
    ...
    femployee_languagesedit.addFields([
    ...
    // Form_CustomValidate
    femployee_languagesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    femployee_languagesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    femployee_languagesedit.lists.reading = <?= $Page->reading->toClientList($Page) ?>;
    femployee_languagesedit.lists.speaking = <?= $Page->speaking->toClientList($Page) ?>;
    femployee_languagesedit.lists.writing = <?= $Page->writing->toClientList($Page) ?>;
    femployee_languagesedit.lists.understanding = <?= $Page->understanding->toClientList($Page) ?>;
    loadjs.done("femployee_languagesedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="femployee_languagesedit" id="femployee_languagesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_employee_languages_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_employee_languages_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="employee_languages" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
