...
// Page object
$CrmLeadstatusEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fcrm_leadstatusedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fcrm_leadstatusedit = currentForm = new ew.Form("fcrm_leadstatusedit", "edit");
    ...
    fcrm_leadstatusedit.addFields([
        ["leadstatusid", [fields.leadstatusid.visible && fields.leadstatusid.required ? ew.Validators.required(fields.leadstatusid.caption) : null], fields.leadstatusid.isInvalid],
    ...
    // Form_CustomValidate
    fcrm_leadstatusedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcrm_leadstatusedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcrm_leadstatusedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcrm_leadstatusedit" id="fcrm_leadstatusedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->leadstatusid->Visible) { // leadstatusid ?>
    <div id="r_leadstatusid" class="form-group row">
        <label id="elh_crm_leadstatus_leadstatusid" class="<?= $Page->LeftColumnClass ?>"><?= $Page->leadstatusid->caption() ?><?= $Page->leadstatusid->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->leadstatusid->cellAttributes() ?>>
<span id="el_crm_leadstatus_leadstatusid">
<span<?= $Page->leadstatusid->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->leadstatusid->getDisplayValue($Page->leadstatusid->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="crm_leadstatus" data-field="x_leadstatusid" data-hidden="1" name="x_leadstatusid" id="x_leadstatusid" value="<?= HtmlEncode($Page->leadstatusid->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
