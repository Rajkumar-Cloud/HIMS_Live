...
// Page object
$PharmacyStoremastEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpharmacy_storemastedit;
loadjs.ready("head", function () {
    ...
    fpharmacy_storemastedit.addFields([
    ...
    // Form_CustomValidate
    fpharmacy_storemastedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpharmacy_storemastedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpharmacy_storemastedit.lists.TYPE = <?= $Page->TYPE->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.LASTSUPP = <?= $Page->LASTSUPP->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.GENNAME = <?= $Page->GENNAME->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.DRID = <?= $Page->DRID->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.MFRCODE = <?= $Page->MFRCODE->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.COMBCODE = <?= $Page->COMBCODE->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.GENCODE = <?= $Page->GENCODE->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.UNIT = <?= $Page->UNIT->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.FORMULARY = <?= $Page->FORMULARY->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.SUPPNAME = <?= $Page->SUPPNAME->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.COMBNAME = <?= $Page->COMBNAME->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.GENERICNAME = <?= $Page->GENERICNAME->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.Scheduling = <?= $Page->Scheduling->toClientList($Page) ?>;
    fpharmacy_storemastedit.lists.Schedulingh1 = <?= $Page->Schedulingh1->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpharmacy_storemastedit" id="fpharmacy_storemastedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fpharmacy_storemastedit"], function() {
    fpharmacy_storemastedit.createAutoSuggest(Object.assign({"id":"x_GENNAME","forceSelect":false}, ew.vars.tables.pharmacy_storemast.fields.GENNAME.autoSuggestOptions));
});
</script>
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pharmacy_storemast_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pharmacy_storemast_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pharmacy_storemast" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
