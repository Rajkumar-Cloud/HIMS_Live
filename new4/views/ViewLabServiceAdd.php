...
// Page object
$ViewLabServiceAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_lab_serviceadd;
loadjs.ready("head", function () {
    ...
    fview_lab_serviceadd.addFields([
    ...
    // Form_CustomValidate
    fview_lab_serviceadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_lab_serviceadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_lab_serviceadd.lists.UNITS = <?= $Page->UNITS->toClientList($Page) ?>;
    fview_lab_serviceadd.lists.SERVICE_TYPE = <?= $Page->SERVICE_TYPE->toClientList($Page) ?>;
    fview_lab_serviceadd.lists.DEPARTMENT = <?= $Page->DEPARTMENT->toClientList($Page) ?>;
    fview_lab_serviceadd.lists.Inactive = <?= $Page->Inactive->toClientList($Page) ?>;
    fview_lab_serviceadd.lists.TestType = <?= $Page->TestType->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_lab_serviceadd" id="fview_lab_serviceadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fview_lab_serviceadd", "editor"], function() {
	ew.createEditor("fview_lab_serviceadd", "x_Interpretation", 35, 4, <?= $Page->Interpretation->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<template id="tpm_view_lab_serviceadd">
<div id="ct_ViewLabServiceAdd"><style>
...
<?php
    if (in_array("view_lab_service_sub", explode(",", $Page->getCurrentDetailTable())) && $view_lab_service_sub->DetailAdd) {
?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_lab_serviceadd", "tpm_view_lab_serviceadd", "view_lab_serviceadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
