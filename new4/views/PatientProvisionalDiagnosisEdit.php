...
loadjs.ready("head", function () {
    ...
    fpatient_provisional_diagnosisedit.lists.provisionaldiagnosisTemplate = <?= $Page->provisionaldiagnosisTemplate->toClientList($Page) ?>;
    fpatient_provisional_diagnosisedit.lists.PatientSearch = <?= $Page->PatientSearch->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpatient_provisional_diagnosisedit" id="fpatient_provisional_diagnosisedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_patient_provisional_diagnosis_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_patient_provisional_diagnosis_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
...
<input type="hidden" data-table="patient_provisional_diagnosis" data-field="x_mrnno" data-hidden="1" name="x_mrnno" id="x_mrnno" value="<?= HtmlEncode($Page->mrnno->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="patient_provisional_diagnosis" data-field="x_Reception" data-hidden="1" name="x_Reception" id="x_Reception" value="<?= HtmlEncode($Page->Reception->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="patient_provisional_diagnosis" data-field="x_PatientId" data-hidden="1" name="x_PatientId" id="x_PatientId" value="<?= HtmlEncode($Page->PatientId->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
loadjs.ready(["fpatient_provisional_diagnosisedit", "editor"], function() {
	ew.createEditor("fpatient_provisional_diagnosisedit", "x_provisional_diagnosis", 35, 25, <?= $Page->provisional_diagnosis->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<template id="tpm_patient_provisional_diagnosisedit">
<div id="ct_PatientProvisionalDiagnosisEdit"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_patient_provisional_diagnosisedit", "tpm_patient_provisional_diagnosisedit", "patient_provisional_diagnosisedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
