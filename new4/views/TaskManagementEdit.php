...
loadjs.ready("head", function () {
    ...
    ftask_managementedit.addFields([
        ...
        ["GetModifiedName", [fields.GetModifiedName.visible && fields.GetModifiedName.required ? ew.Validators.required(fields.GetModifiedName.caption) : null], fields.GetModifiedName.isInvalid],
    ...
    ftask_managementedit.lists.AssignTo = <?= $Page->AssignTo->toClientList($Page) ?>;
    ftask_managementedit.lists.StatusOfTask = <?= $Page->StatusOfTask->toClientList($Page) ?>;
    ftask_managementedit.lists.ForwardTo = <?= $Page->ForwardTo->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="ftask_managementedit" id="ftask_managementedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["ftask_managementedit", "editor"], function() {
	ew.createEditor("ftask_managementedit", "x_Description", 35, 4, <?= $Page->Description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
