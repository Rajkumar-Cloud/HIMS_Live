...
// Page object
$PharmacyMasterGenericAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpharmacy_master_genericadd;
loadjs.ready("head", function () {
    ...
    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "pharmacy_master_generic")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.pharmacy_master_generic)
        ew.vars.tables.pharmacy_master_generic = currentTable;
    fpharmacy_master_genericadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fpharmacy_master_genericadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpharmacy_master_genericadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpharmacy_master_genericadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpharmacy_master_genericadd.lists.GRPCODE = <?= $Page->GRPCODE->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpharmacy_master_genericadd" id="fpharmacy_master_genericadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" is="selection-list" data-table="pharmacy_master_generic" data-field="x_GRPCODE" data-type="text" data-multiple="0" data-lookup="1" data-value-separator="<?= $Page->GRPCODE->displayValueSeparatorAttribute() ?>" name="x_GRPCODE" id="x_GRPCODE" value="<?= $Page->GRPCODE->CurrentValue ?>"<?= $Page->GRPCODE->editAttributes() ?>>
</span>
</div></div>
    </div>
<?php } ?>
