...
// Page object
$PatientEmergencyContactEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpatient_emergency_contactedit;
loadjs.ready("head", function () {
    ...
    fpatient_emergency_contactedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fpatient_emergency_contactedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpatient_emergency_contactedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpatient_emergency_contactedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpatient_emergency_contactedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpatient_emergency_contactedit" id="fpatient_emergency_contactedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_patient_emergency_contact_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_patient_emergency_contact_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="patient_emergency_contact" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="patient_emergency_contact" data-field="x_patient_id" data-hidden="1" name="x_patient_id" id="x_patient_id" value="<?= HtmlEncode($Page->patient_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_patient_emergency_contact_status">
    <select
        ...
        data-select2-id="patient_emergency_contact_x_status"
...
<script>
loadjs.ready("head", function() {
    var el = document.querySelector("select[data-select2-id='patient_emergency_contact_x_status']"),
        options = { name: "x_status", selectId: "patient_emergency_contact_x_status", language: ew.LANGUAGE_ID, dir: ew.IS_RTL ? "rtl" : "ltr" };
