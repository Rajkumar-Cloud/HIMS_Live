...
// Page object
$ViewIpPatientAdmissionAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_ip_patient_admissionadd;
loadjs.ready("head", function () {
    ...
    fview_ip_patient_admissionadd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fview_ip_patient_admissionadd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fview_ip_patient_admissionadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_ip_patient_admissionadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_ip_patient_admissionadd.lists.typeRegsisteration = <?= $Page->typeRegsisteration->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.PaymentCategory = <?= $Page->PaymentCategory->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.PaymentStatus = <?= $Page->PaymentStatus->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.HospID = <?= $Page->HospID->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.ReferedByDr = <?= $Page->ReferedByDr->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.BillClosing = <?= $Page->BillClosing->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.ReportHeader = <?= $Page->ReportHeader->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.Procedure = <?= $Page->Procedure->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.Consultant = <?= $Page->Consultant->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.Anesthetist = <?= $Page->Anesthetist->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.physician_id = <?= $Page->physician_id->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.patient_id = <?= $Page->patient_id->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.Cid = <?= $Page->Cid->toClientList($Page) ?>;
    fview_ip_patient_admissionadd.lists.AdviceToOtherHospital = <?= $Page->AdviceToOtherHospital->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_ip_patient_admissionadd" id="fview_ip_patient_admissionadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<template id="tpm_view_ip_patient_admissionadd">
<div id="ct_ViewIpPatientAdmissionAdd"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_ip_patient_admissionadd", "tpm_view_ip_patient_admissionadd", "view_ip_patient_admissionadd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
