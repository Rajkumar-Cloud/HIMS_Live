...
// Page object
$ViewBillingVoucherAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_billing_voucheradd;
loadjs.ready("head", function () {
    ...
    fview_billing_voucheradd.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fview_billing_voucheradd,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fview_billing_voucheradd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_billing_voucheradd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_billing_voucheradd.lists.Reception = <?= $Page->Reception->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.ModeofPayment = <?= $Page->ModeofPayment->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.CId = <?= $Page->CId->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.PatId = <?= $Page->PatId->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.RefferedBy = <?= $Page->RefferedBy->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.DrID = <?= $Page->DrID->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.ReportHeader = <?= $Page->ReportHeader->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.AdjustmentAdvance = <?= $Page->AdjustmentAdvance->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.BillType = <?= $Page->BillType->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.IncludePackage = <?= $Page->IncludePackage->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.CancelBill = <?= $Page->CancelBill->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.BillClosing = <?= $Page->BillClosing->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.GoogleReviewID = <?= $Page->GoogleReviewID->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.CardType = <?= $Page->CardType->toClientList($Page) ?>;
    fview_billing_voucheradd.lists.PharmacyBill = <?= $Page->PharmacyBill->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_billing_voucheradd" id="fview_billing_voucheradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="<?= $Page->sid->getInputTextType() ?>" data-table="view_billing_voucher" data-field="x_sid" name="x_sid" id="x_sid" size="30" placeholder="<?= HtmlEncode($Page->sid->getPlaceHolder()) ?>" value="<?= $Page->sid->EditValue ?>"<?= $Page->sid->editAttributes() ?> aria-describedby="x_sid_help">
...
<input type="<?= $Page->SidNo->getInputTextType() ?>" data-table="view_billing_voucher" data-field="x_SidNo" name="x_SidNo" id="x_SidNo" size="30" maxlength="45" placeholder="<?= HtmlEncode($Page->SidNo->getPlaceHolder()) ?>" value="<?= $Page->SidNo->EditValue ?>"<?= $Page->SidNo->editAttributes() ?> aria-describedby="x_SidNo_help">
...
<template id="tpm_view_billing_voucheradd">
<div id="ct_ViewBillingVoucherAdd"><style>
...
<script src="customized\lightWeightPopup\lightWeightPopup.js"></script>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_billing_voucheradd", "tpm_view_billing_voucheradd", "view_billing_voucheradd", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
