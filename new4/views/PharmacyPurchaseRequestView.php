...
<?php } ?>
<script>
if (!ew.vars.tables.pharmacy_purchase_request) ew.vars.tables.pharmacy_purchase_request = <?= JsonEncode(GetClientVar("tables", "pharmacy_purchase_request")) ?>;
</script>
<?php if (!$Page->isExport()) { ?>
...
$Page->showMessage();
?>
<form name="fpharmacy_purchase_requestview" id="fpharmacy_purchase_requestview" class="form-inline ew-form ew-view-form" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php
    if (in_array("pharmacy_purchase_request_items", explode(",", $Page->getCurrentDetailTable())) && $pharmacy_purchase_request_items->DetailView) {
?>
