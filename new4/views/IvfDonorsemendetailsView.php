...
$Page->showMessage();
?>
<form name="fivf_donorsemendetailsview" id="fivf_donorsemendetailsview" class="form-inline ew-form ew-view-form" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->Lagency->Visible) { // Lagency ?>
    <tr id="r_Lagency">
        <td class="<?= $Page->TableLeftColumnClass ?>"><span id="elh_ivf_donorsemendetails_Lagency"><?= $Page->Lagency->caption() ?></span></td>
        <td data-name="Lagency" <?= $Page->Lagency->cellAttributes() ?>>
<span id="el_ivf_donorsemendetails_Lagency">
<span<?= $Page->Lagency->viewAttributes() ?>>
<?= $Page->Lagency->getViewValue() ?></span>
</span>
</td>
    </tr>
<?php } ?>
