...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    freceptionadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    freceptionadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    freceptionadd.lists.CATEGORY = <?= $Page->CATEGORY->toClientList($Page) ?>;
    freceptionadd.lists.PROCEDURE = <?= $Page->PROCEDURE->toClientList($Page) ?>;
    freceptionadd.lists.MODEOFPAYMENT = <?= $Page->MODEOFPAYMENT->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="freceptionadd" id="freceptionadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["freceptionadd", "datetimepicker"], function() {
    ew.createDateTimePicker("freceptionadd", "x_NEXTREVIEWDATE", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
