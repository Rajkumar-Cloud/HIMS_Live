...
// Page object
$PresMasFormsEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpres_mas_formsedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var currentTable = <?= JsonEncode(GetClientVar("tables", "pres_mas_forms")) ?>,
        fields = currentTable.fields;
    if (!ew.vars.tables.pres_mas_forms)
        ew.vars.tables.pres_mas_forms = currentTable;
    fpres_mas_formsedit.addFields([
    ...
    // Form_CustomValidate
    fpres_mas_formsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpres_mas_formsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fpres_mas_formsedit" id="fpres_mas_formsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pres_mas_forms_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pres_mas_forms_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pres_mas_forms" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
