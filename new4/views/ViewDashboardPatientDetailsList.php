...
loadjs.ready("head", function () {
    ...
    fview_dashboard_patient_detailslistsrch.addFields([
    ...
    // Set invalid fields
    $(function() {
        fview_dashboard_patient_detailslistsrch.setInvalid();
    });

    // Validate form
    fview_dashboard_patient_detailslistsrch.validate = function () {
    ...
    // Form_CustomValidate
    fview_dashboard_patient_detailslistsrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_dashboard_patient_detailslistsrch.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
<?php
if ($Page->DbMasterFilter != "" && $Page->getCurrentMasterTable() == "view_dashboard_patient_summary") {
    if ($Page->MasterRecordExists) {
        include_once "views/ViewDashboardPatientSummaryMaster.php";
    }
}
?>
<?php } ?>
...
<form name="fview_dashboard_patient_detailslistsrch" id="fview_dashboard_patient_detailslistsrch" class="form-inline ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>">
...
<input type="<?= $Page->createdDate->getInputTextType() ?>" data-table="view_dashboard_patient_details" data-field="x_createdDate" data-format="7" name="x_createdDate" id="x_createdDate" placeholder="<?= HtmlEncode($Page->createdDate->getPlaceHolder()) ?>" value="<?= $Page->createdDate->EditValue ?>"<?= $Page->createdDate->editAttributes() ?>>
...
<input type="<?= $Page->createdDate->getInputTextType() ?>" data-table="view_dashboard_patient_details" data-field="x_createdDate" data-format="7" name="y_createdDate" id="y_createdDate" placeholder="<?= HtmlEncode($Page->createdDate->getPlaceHolder()) ?>" value="<?= $Page->createdDate->EditValue2 ?>"<?= $Page->createdDate->editAttributes() ?>>
<div class="invalid-feedback"><?= $Page->createdDate->getErrorMessage(false) ?></div>
<?php if (!$Page->createdDate->ReadOnly && !$Page->createdDate->Disabled && !isset($Page->createdDate->EditAttrs["readonly"]) && !isset($Page->createdDate->EditAttrs["disabled"])) { ?>
<script>
loadjs.ready(["fview_dashboard_patient_detailslistsrch", "datetimepicker"], function() {
...
<?php } ?>
<form name="fview_dashboard_patient_detailslist" id="fview_dashboard_patient_detailslist" class="form-inline ew-form ew-list-form" action="<?= CurrentPageUrl(false) ?>" method="post">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="view_dashboard_patient_details">
<?php if ($Page->getCurrentMasterTable() == "view_dashboard_patient_summary" && $Page->CurrentAction) { ?>
<input type="hidden" name="<?= Config("TABLE_SHOW_MASTER") ?>" value="view_dashboard_patient_summary">
<input type="hidden" name="fk_createdDate" value="<?= HtmlEncode($Page->createdDate->getSessionValue()) ?>">
<input type="hidden" name="fk_WhereDidYouHear" value="<?= HtmlEncode($Page->WhereDidYouHear->getSessionValue()) ?>">
<?php } ?>
