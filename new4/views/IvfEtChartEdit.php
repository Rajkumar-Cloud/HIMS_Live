...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fivf_et_chartedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fivf_et_chartedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fivf_et_chartedit.lists.ARTCycle = <?= $Page->ARTCycle->toClientList($Page) ?>;
    fivf_et_chartedit.lists.InseminatinTechnique = <?= $Page->InseminatinTechnique->toClientList($Page) ?>;
    fivf_et_chartedit.lists.PreTreatment = <?= $Page->PreTreatment->toClientList($Page) ?>;
    fivf_et_chartedit.lists.Hysteroscopy = <?= $Page->Hysteroscopy->toClientList($Page) ?>;
    fivf_et_chartedit.lists.EndometrialScratching = <?= $Page->EndometrialScratching->toClientList($Page) ?>;
    fivf_et_chartedit.lists.TrialCannulation = <?= $Page->TrialCannulation->toClientList($Page) ?>;
    fivf_et_chartedit.lists.CYCLETYPE = <?= $Page->CYCLETYPE->toClientList($Page) ?>;
    fivf_et_chartedit.lists.OralEstrogenDosage = <?= $Page->OralEstrogenDosage->toClientList($Page) ?>;
    fivf_et_chartedit.lists.GCSF = <?= $Page->GCSF->toClientList($Page) ?>;
    fivf_et_chartedit.lists.ActivatedPRP = <?= $Page->ActivatedPRP->toClientList($Page) ?>;
    fivf_et_chartedit.lists.ERA = <?= $Page->ERA->toClientList($Page) ?>;
    fivf_et_chartedit.lists.TemplateProcedureRecord = <?= $Page->TemplateProcedureRecord->toClientList($Page) ?>;
    fivf_et_chartedit.lists.TemplateMedicationsadvised = <?= $Page->TemplateMedicationsadvised->toClientList($Page) ?>;
    fivf_et_chartedit.lists.TemplatePostProcedureInstructions = <?= $Page->TemplatePostProcedureInstructions->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fivf_et_chartedit" id="fivf_et_chartedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_ivf_et_chart_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_ivf_et_chart_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<template id="tpx_ivf_et_chart_id"><span id="el_ivf_et_chart_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span></template>
<input type="hidden" data-table="ivf_et_chart" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
loadjs.ready(["fivf_et_chartedit", "editor"], function() {
	ew.createEditor("fivf_et_chartedit", "x_ProcedureRecord", 35, 4, <?= $Page->ProcedureRecord->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<script>
loadjs.ready(["fivf_et_chartedit", "editor"], function() {
	ew.createEditor("fivf_et_chartedit", "x_Medicationsadvised", 35, 4, <?= $Page->Medicationsadvised->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<script>
loadjs.ready(["fivf_et_chartedit", "editor"], function() {
	ew.createEditor("fivf_et_chartedit", "x_PostProcedureInstructions", 35, 4, <?= $Page->PostProcedureInstructions->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span></template>
</div></div>
...
<template id="tpm_ivf_et_chartedit">
<div id="ct_IvfEtChartEdit"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_ivf_et_chartedit", "tpm_ivf_et_chartedit", "ivf_et_chartedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
