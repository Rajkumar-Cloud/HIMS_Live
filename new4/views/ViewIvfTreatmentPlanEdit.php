...
// Page object
$ViewIvfTreatmentPlanEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fview_ivf_treatment_planedit;
loadjs.ready("head", function () {
    ...
    fview_ivf_treatment_planedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fview_ivf_treatment_planedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fview_ivf_treatment_planedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fview_ivf_treatment_planedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fview_ivf_treatment_planedit.lists.RIDNO = <?= $Page->RIDNO->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.treatment_status = <?= $Page->treatment_status->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.ARTCYCLE = <?= $Page->ARTCYCLE->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.RESULT = <?= $Page->RESULT->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.Treatment = <?= $Page->Treatment->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.TypeOfCycle = <?= $Page->TypeOfCycle->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.SpermOrgin = <?= $Page->SpermOrgin->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.State = <?= $Page->State->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.MACS = <?= $Page->MACS->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.PgdPlanning = <?= $Page->PgdPlanning->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.MaleIndications = <?= $Page->MaleIndications->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.FemaleIndications = <?= $Page->FemaleIndications->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.Heterotopic = <?= $Page->Heterotopic->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.TransferDFE = <?= $Page->TransferDFE->toClientList($Page) ?>;
    fview_ivf_treatment_planedit.lists.TineLapse = <?= $Page->TineLapse->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fview_ivf_treatment_planedit" id="fview_ivf_treatment_planedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_view_ivf_treatment_plan_id" class="<?= $Page->LeftColumnClass ?>"><template id="tpc_view_ivf_treatment_plan_id"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></template></label>
...
<input type="hidden" data-table="view_ivf_treatment_plan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<template id="tpm_view_ivf_treatment_planedit">
<div id="ct_ViewIvfTreatmentPlanEdit"><style>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_view_ivf_treatment_planedit", "tpm_view_ivf_treatment_planedit", "view_ivf_treatment_planedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
