...
// Page object
$PresFrequenciesEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpres_frequenciesedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.pres_frequencies.fields;
    fpres_frequenciesedit.addFields([
        ...
        ["Freq_Exp", [fields.Freq_Exp.required ? ew.Validators.required(fields.Freq_Exp.caption) : null], fields.Freq_Exp.isInvalid],
        ["Freq_Count", [fields.Freq_Count.required ? ew.Validators.required(fields.Freq_Count.caption) : null, ew.Validators.integer], fields.Freq_Count.isInvalid]
    ]);

    // Set invalid fields
    $(function() {
        var f = fpres_frequenciesedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpres_frequenciesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpres_frequenciesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fpres_frequenciesedit" id="fpres_frequenciesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pres_frequencies_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pres_frequencies_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pres_frequencies" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
