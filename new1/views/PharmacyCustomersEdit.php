...
// Page object
$PharmacyCustomersEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpharmacy_customersedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fpharmacy_customersedit = currentForm = new ew.Form("fpharmacy_customersedit", "edit");

    // Add fields
    var fields = ew.vars.tables.pharmacy_customers.fields;
    fpharmacy_customersedit.addFields([
        ["Customercode", [fields.Customercode.required ? ew.Validators.required(fields.Customercode.caption) : null, ew.Validators.integer], fields.Customercode.isInvalid],
        ...
        ["Ratetype", [fields.Ratetype.required ? ew.Validators.required(fields.Ratetype.caption) : null], fields.Ratetype.isInvalid],
        ...
        ["ContactPerson", [fields.ContactPerson.required ? ew.Validators.required(fields.ContactPerson.caption) : null], fields.ContactPerson.isInvalid],
        ["CPPhone", [fields.CPPhone.required ? ew.Validators.required(fields.CPPhone.caption) : null], fields.CPPhone.isInvalid],
        ["id", [fields.id.required ? ew.Validators.required(fields.id.caption) : null], fields.id.isInvalid]
    ]);

    // Set invalid fields
    $(function() {
        var f = fpharmacy_customersedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fpharmacy_customersedit.validate = function () {
    ...
    // Form_CustomValidate
    fpharmacy_customersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpharmacy_customersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fpharmacy_customersedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpharmacy_customersedit" id="fpharmacy_customersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<script>
loadjs.ready(["fpharmacy_customersedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fpharmacy_customersedit", "x_Creationdate", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pharmacy_customers_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pharmacy_customers_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pharmacy_customers" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
