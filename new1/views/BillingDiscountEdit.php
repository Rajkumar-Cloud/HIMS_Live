...
// Page object
$BillingDiscountEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fbilling_discountedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.billing_discount.fields;
    fbilling_discountedit.addFields([
        ["id", [fields.id.required ? ew.Validators.required(fields.id.caption) : null], fields.id.isInvalid],
        ["Particulars", [fields.Particulars.required ? ew.Validators.required(fields.Particulars.caption) : null], fields.Particulars.isInvalid],
        ["Procedure", [fields.Procedure.required ? ew.Validators.required(fields.Procedure.caption) : null, ew.Validators.float], fields.Procedure.isInvalid],
        ["Pharmacy", [fields.Pharmacy.required ? ew.Validators.required(fields.Pharmacy.caption) : null, ew.Validators.float], fields.Pharmacy.isInvalid],
        ["Investication", [fields.Investication.required ? ew.Validators.required(fields.Investication.caption) : null, ew.Validators.float], fields.Investication.isInvalid]
    ]);

    // Set invalid fields
    $(function() {
        var f = fbilling_discountedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fbilling_discountedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbilling_discountedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fbilling_discountedit" id="fbilling_discountedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_billing_discount_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_billing_discount_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="billing_discount" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
