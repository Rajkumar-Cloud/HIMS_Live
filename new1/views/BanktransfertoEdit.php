...
// Page object
$BanktransfertoEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fbanktransfertoedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.banktransferto.fields;
    fbanktransfertoedit.addFields([
        ...
        ["Mobile_Number", [fields.Mobile_Number.required ? ew.Validators.required(fields.Mobile_Number.caption) : null], fields.Mobile_Number.isInvalid],
        ["AccountNo", [fields.AccountNo.required ? ew.Validators.required(fields.AccountNo.caption) : null], fields.AccountNo.isInvalid]
    ]);

    // Set invalid fields
    $(function() {
        var f = fbanktransfertoedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fbanktransfertoedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fbanktransfertoedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fbanktransfertoedit" id="fbanktransfertoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_banktransferto_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_banktransferto_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="banktransferto" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
