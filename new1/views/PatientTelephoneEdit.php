...
// Page object
$PatientTelephoneEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpatient_telephoneedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.patient_telephone.fields;
    fpatient_telephoneedit.addFields([
    ...
    // Form_CustomValidate
    fpatient_telephoneedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpatient_telephoneedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fpatient_telephoneedit" id="fpatient_telephoneedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_patient_telephone_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_patient_telephone_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="patient_telephone" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="<?= $Page->tele_type->getInputTextType() ?>" data-table="patient_telephone" data-field="x_tele_type" name="x_tele_type" id="x_tele_type" size="30" placeholder="<?= HtmlEncode($Page->tele_type->getPlaceHolder()) ?>" value="<?= $Page->tele_type->EditValue ?>"<?= $Page->tele_type->editAttributes() ?> aria-describedby="x_tele_type_help">
...
<input type="<?= $Page->status->getInputTextType() ?>" data-table="patient_telephone" data-field="x_status" name="x_status" id="x_status" size="30" placeholder="<?= HtmlEncode($Page->status->getPlaceHolder()) ?>" value="<?= $Page->status->EditValue ?>"<?= $Page->status->editAttributes() ?> aria-describedby="x_status_help">
