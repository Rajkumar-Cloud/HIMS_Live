...
// Page object
$PharmacyCombMasterEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpharmacy_comb_masteredit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.pharmacy_comb_master.fields;
    fpharmacy_comb_masteredit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fpharmacy_comb_masteredit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpharmacy_comb_masteredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpharmacy_comb_masteredit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fpharmacy_comb_masteredit" id="fpharmacy_comb_masteredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="<?= $Page->GRPCODE->getInputTextType() ?>" data-table="pharmacy_comb_master" data-field="x_GRPCODE" name="x_GRPCODE" id="x_GRPCODE" size="30" maxlength="5" placeholder="<?= HtmlEncode($Page->GRPCODE->getPlaceHolder()) ?>" value="<?= $Page->GRPCODE->EditValue ?>"<?= $Page->GRPCODE->editAttributes() ?> aria-describedby="x_GRPCODE_help">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pharmacy_comb_master_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pharmacy_comb_master_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pharmacy_comb_master" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
</div><!-- /page* -->
