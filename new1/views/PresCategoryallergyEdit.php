...
// Page object
$PresCategoryallergyEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpres_categoryallergyedit;
loadjs.ready("head", function () {
    ...
    // Add fields
    var fields = ew.vars.tables.pres_categoryallergy.fields;
    fpres_categoryallergyedit.addFields([
        ...
        ["CategoryDrug", [fields.CategoryDrug.required ? ew.Validators.required(fields.CategoryDrug.caption) : null], fields.CategoryDrug.isInvalid]
    ]);

    // Set invalid fields
    $(function() {
        var f = fpres_categoryallergyedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Form_CustomValidate
    fpres_categoryallergyedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpres_categoryallergyedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
$Page->showMessage();
?>
<form name="fpres_categoryallergyedit" id="fpres_categoryallergyedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_pres_categoryallergy_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_pres_categoryallergy_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pres_categoryallergy" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
