...
if($control == "FindivfCoupleReg")
{
	...
	if($Patientid == '')
		...
		{
			$sql = "SELECT * FROM ganeshkumar_bjhims.ip_admission where patient_id='".$Patientid."'  and BillClosing is null  or BillClosing != 'Yes'  order by id desc limit 1;";
		$result=$conn->query($sql);
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
...
if($control == "GetOPAdvance")
{
	...
	foreach($obj as $item) {
		...
		$sql = "SELECT sum(Amount) as TotalAdvance  FROM ganeshkumar_bjhims.billing_other_voucher where PatID='".$patientId."'   and HospID='".$HospID."'  and  (freezed != 'Yes' or   freezed is null )     ;";
...
if($control == "TillSearch")
{
	...
	foreach($obj as $item) {
		...
		$sql1 = "SELECT * FROM ganeshkumar_bjhims.depositdetails where ModifiedUserName = '".$CurrentUserName."' and DepositDate = DATE('".$FromDate."')  and CreatedDateTime between '".$FromDate."' and '".$ToDate."' and HospID='".$CurrentUserID."';";
...
$sql = "SELECT sum(amount) as Amount ,ModeofPayment , sum(Cash) as Cash,sum(Card) as Card FROM ganeshkumar_bjhims.view_till_search
...
if($control == "ServiceWiseBillNo")
{
	...
	foreach($obj as $item) {	
		...
		$sql =  "SELECT createdDate,Services, PatientId,PatientName , Mobile, amount, Vid,invoiceId,invoiceAmount , sid, ItemCode FROM
...
HospID='".$HospitalID."'  and    Service_Department =  '".$Itemmed."'"  ;
...
if($control == "ServiceWiseBillNoIIPP")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
		...
		$sql = "SELECT createdDate,`Procedure`  as  Services, a.PatientId,PatientName , Mobile,a.Amound as amount,b.id as Vid,b.id as invoiceId,
		a.Amound as invoiceAmount , sid, '' as ItemCode FROM ganeshkumar_bjhims.ip_admission a 


INNER JOIN ganeshkumar_bjhims.view_dash_billing_voucher b ON a.BillNumber=b.BillNumber

where a.BillNumber in (SELECT BillNumber FROM ganeshkumar_bjhims.view_dash_billing_voucher
where (HospID='".$HospitalID."' and createddate between '".$fromdte."'  and '".$todate."'  and BillType =  'IP' 
and CancelBill not like '%Cancel%' )  ) and  `Procedure` is not null and `Procedure` = '".$Itemmed ."'  group by `Procedure` ,
createdDate,`Procedure`  , a.PatientId,PatientName , Mobile, amount,b.id ,b.id ,a.Amound  , sid ;;";											
...
where (HospID='".$HospitalID."' and createddate between '".$fromdte."' and '".$todate."' and BillType =  '".$Itemmed."'
and CancelBill not like '%Cancel%' )  ) and amount>0 group by SERVICE";
...
if($control == "ServiceWiseBillNoABBBB")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
...
$sql = "SELECT voucher_type as SERVICE ,sum(amount) as Amount ,count(voucher_type) as Count FROM ganeshkumar_bjhims.view_pharmacy_billing_voucher where
HospID='".$HospitalID."' and createddate between '".$fromdte."' and '".$todate."'
...
if($control == "ServiceWiseBillNoB")
{
	...
	foreach($obj as $item) {	
		...
		$sql =  "SELECT createddate, ProcedureName, PatientId,PatientName , Mobile, amount, id as Vid, BillNumber as invoiceId,
		 Amount as invoiceAmount , sid, '' as ItemCode

 FROM ganeshkumar_bjhims.view_dash_billing_voucher
	where HospID='".$HospitalID."' and createddate between '".$fromdte."' and '".$todate."' and CancelBill not like '%Cancel%'
	and  BillType = '".$Itemmed."'";
...
if($control == "StoreGRNSearch")
{
	...
	foreach($obj as $item) {	
		$PC = $item['PC'];
		$HospitalID = $item['HospitalID'];
		$sql = "SELECT * FROM ganeshkumar_bjhims.store_grn where Customername='".$PC."' and Pid is null and HospID = '".$HospitalID."';";		
		$result=$conn->query($sql);
		$products_arr["records"]=array();
		if ($result->num_rows > 0) {
			// output data of each row
...
if($control == "ServiceWiseBillNoArevenue")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
...
if($Itemmed  == 'OP')
{
$sql = "SELECT DEPARTMENT ,  sum(amount) as Amount , count(Services) as Count FROM ganeshkumar_bjhims.view_dashboard_service_details
where 
vid in  (SELECT id FROM ganeshkumar_bjhims.view_dash_billing_voucher
where (HospID='".$HospitalID."' and createddate between '".$fromdte."' and '".$todate."' and BillType =  '".$Itemmed."'
and CancelBill not like '%Cancel%' )  ) and amount>0 group by DEPARTMENT ;" ;
}else
{
	$sql = "SELECT `Procedure` as DEPARTMENT,  sum(Amound) as Amount , count(`Procedure`) as Count FROM ganeshkumar_bjhims.ip_admission
where BillNumber in (SELECT BillNumber FROM ganeshkumar_bjhims.view_dash_billing_voucher
where (HospID='2' and createddate between '".$fromdte."' and '".$todate."'  and BillType =  'IP'
and CancelBill not like '%Cancel%' )  ) and `Procedure` is not null  group by `Procedure`  ;";
}
...
if($control == "ServiceWiseBillNoArevenuePharmachy")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
...
$sql = "SELECT BillType as SERVICE ,sum(amount) as Amount ,count(BillType) as Count FROM ganeshkumar_bjhims.pharmacy_billing_voucher where
HospID='".$HospitalID."' and createddatetime between '".$fromdte."' and '".$todate."' 
group by BillType
;" ;
...
if($control == "ServiceWiseBillNoPharmacy")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
		...
		$sql =  "SELECT  PatientId,PatientName , Mobile, amount, id,Amount , BillType FROM

 ganeshkumar_bjhims.pharmacy_billing_voucher
where HospID='".$HospitalID."'  and createddatetime between '".$fromdte."'  and '".$todate."' 
and  BillType =  '".$Itemmed."'"  ;
...
if($control == "ServiceWiseBillNoArevenueAAA")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
...
$sql = "SELECT SERVICE_TYPE ,  sum(amount) as Amount , count(Services) as Count FROM ganeshkumar_bjhims.view_dashboard_service_details
where 
vid in  (SELECT id FROM ganeshkumar_bjhims.view_dash_billing_voucher
where (HospID='".$HospitalID."' and createddate between '".$fromdte."' and '".$todate."' and BillType =  '".$Itemmed."'
and CancelBill not like '%Cancel%' )  ) and amount>0 group by SERVICE_TYPE ;" ;
...
if($control == "ServiceWiseBillNoBBBBBB")
{
	$data = $_GET['data'];
	$obj = json_decode($data,true);
	foreach($obj as $item) {	
...
(HospID='".$HospitalID."'  and createddate between '".$fromdte."'  and '".$todate."' 
and CancelBill not like '%Cancel%' and IncludePackage != 'Yes')) and
HospID='".$HospitalID."'  and    Service_Type =  '".$Itemmed."'"  ;
