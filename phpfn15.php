...
function CurrentProjectID() {
	...
	return "{EA4F6D13-AF8F-428D-9EA7-BA88D1107EC3}";
}
...
class Breadcrumb
{
	...
	{
		global $Language;
		$this->Links[] = ["home", "HomePage", "dashboard.php", "ew-home", "", FALSE]; // Home
	}
...
class AdvancedSecurity
{
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setCurrentUserName($rs->fields('mail_id')); // Load user name
	...
	{
		return $this->currentUserInfo("mail_id");
	}
...
function Barcode() {
	...
	if ($Barcode === NULL) {
		...
		$Barcode->UsePhpExcel = defined(PROJECT_NAMESPACE . "USE_PHPEXCEL");
		$Barcode->UsePhpWord = defined(PROJECT_NAMESPACE . "USE_PHPWORD");
...
if ($item->Text == "Doctor Notes Excel" || $item->Text == "Treatment Status" || $item->Text == "Monitor Treatment Plan" || $item->Text == "Bill Collection Report" ||
$item->Text == "Diff Bills" || $item->Text == "Collection Summary" || $item->Text == "Monitor Treatment Plan" || $item->Text == "Procedure Registered" ||
$item->Text == "Service" ||  $item->Text == "Collection" ||  $item->Text == "Dash Board" ||  $item->Text == "Procedure List" ||  
$item->Text == "Appointment" ||  $item->Text == "New Patient" ||  $item->Text == "IP Admission" ||  $item->Text == "IUI Excel" ||  
$item->Text == "Patient Registration" ||  $item->Text == "Doctor Notes" ||  $item->Text == "Discharge Summary" ||  $item->Text == "Follow Ups" ||  
$item->Text == "Fertility" ||  $item->Text == "Embryology" ||  $item->Text == "Andrology" ||  $item->Text == "Deposit" ||  
$item->Text == "Issue" ||  $item->Text == "Billing" ||  $item->Text == "Lab" ||  $item->Text == "Consumption" ||  
$item->Text == "Transfer" ||  $item->Text == "Batch Master" ||  $item->Text == "Pharmacy" ||  $item->Text == "Prescription" ||  
$item->Text == "Delivery Registered" || $item->Text == "Treatment Advised" || $item->Text == "Review Date" || $item->Text == "Patient")
	{
...
if ($item->Text == "Pharmacy")
		...
		if(CurrentUserLevel() == '24')  ////AFPharmacy    ------  pharmacy@a4fertility.com
		{
			...
			if ($item->Text == "Billing")
			{
				...
				if(CurrentUserLevel() == '20')  ////AFPharmacy    ------  pharmacy@a4fertility.com
		{
			if ($item->Text == "Pharmacy")
			{
				return false;
			}
				if ($item->Text == "Report Revenue")
			{
				return false;
			}
				if ($item->Text == "Till Collection Summary")
			{
				return false;
			}
				if ($item->Text == "Diff Bills")
