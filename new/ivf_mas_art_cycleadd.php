...
<script>
var fivf_mas_art_cycleadd, currentPageID;
loadjs.ready("head", function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($ivf_mas_art_cycle_add->ARTCYCLE->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $ivf_mas_art_cycle_add->ARTCYCLE->caption(), $ivf_mas_art_cycle_add->ARTCYCLE->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fivf_mas_art_cycleadd.validateRequired = <?php echo Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
<?php if ($ivf_mas_art_cycle_add->ARTCYCLE->Visible) { // ARTCYCLE ?>
	<div id="r_ARTCYCLE" class="form-group row">
		<label id="elh_ivf_mas_art_cycle_ARTCYCLE" for="x_ARTCYCLE" class="<?php echo $ivf_mas_art_cycle_add->LeftColumnClass ?>"><?php echo $ivf_mas_art_cycle_add->ARTCYCLE->caption() ?><?php echo $ivf_mas_art_cycle_add->ARTCYCLE->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $ivf_mas_art_cycle_add->RightColumnClass ?>"><div <?php echo $ivf_mas_art_cycle_add->ARTCYCLE->cellAttributes() ?>>
<span id="el_ivf_mas_art_cycle_ARTCYCLE">
<input type="text" data-table="ivf_mas_art_cycle" data-field="x_ARTCYCLE" name="x_ARTCYCLE" id="x_ARTCYCLE" size="30" maxlength="45" placeholder="<?php echo HtmlEncode($ivf_mas_art_cycle_add->ARTCYCLE->getPlaceHolder()) ?>" value="<?php echo $ivf_mas_art_cycle_add->ARTCYCLE->EditValue ?>"<?php echo $ivf_mas_art_cycle_add->ARTCYCLE->editAttributes() ?>>
</span>
<?php echo $ivf_mas_art_cycle_add->ARTCYCLE->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
