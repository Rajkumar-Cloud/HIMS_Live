...
<script>
var fpres_fluidsadd, currentPageID;
loadjs.ready("head", function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($pres_fluids_add->FORMS->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pres_fluids_add->FORMS->caption(), $pres_fluids_add->FORMS->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fpres_fluidsadd.validateRequired = <?php echo Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
<?php if ($pres_fluids_add->FORMS->Visible) { // FORMS ?>
	<div id="r_FORMS" class="form-group row">
		<label id="elh_pres_fluids_FORMS" for="x_FORMS" class="<?php echo $pres_fluids_add->LeftColumnClass ?>"><?php echo $pres_fluids_add->FORMS->caption() ?><?php echo $pres_fluids_add->FORMS->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_fluids_add->RightColumnClass ?>"><div <?php echo $pres_fluids_add->FORMS->cellAttributes() ?>>
<span id="el_pres_fluids_FORMS">
<input type="text" data-table="pres_fluids" data-field="x_FORMS" name="x_FORMS" id="x_FORMS" size="30" maxlength="50" placeholder="<?php echo HtmlEncode($pres_fluids_add->FORMS->getPlaceHolder()) ?>" value="<?php echo $pres_fluids_add->FORMS->EditValue ?>"<?php echo $pres_fluids_add->FORMS->editAttributes() ?>>
</span>
<?php echo $pres_fluids_add->FORMS->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
