...
<script>
var fivf_mas_template_typeadd, currentPageID;
loadjs.ready("head", function() {

	// Form object
	currentPageID = ew.PAGE_ID = "add";
	fivf_mas_template_typeadd = currentForm = new ew.Form("fivf_mas_template_typeadd", "add");
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($ivf_mas_template_type_add->TemplateName->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $ivf_mas_template_type_add->TemplateName->caption(), $ivf_mas_template_type_add->TemplateName->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fivf_mas_template_typeadd.validateRequired = <?php echo Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

	// Dynamic selection lists
	loadjs.done("fivf_mas_template_typeadd");
});
</script>
<script>
loadjs.ready("head", function() {
...
<?php if ($ivf_mas_template_type_add->TemplateName->Visible) { // TemplateName ?>
	<div id="r_TemplateName" class="form-group row">
		<label id="elh_ivf_mas_template_type_TemplateName" for="x_TemplateName" class="<?php echo $ivf_mas_template_type_add->LeftColumnClass ?>"><?php echo $ivf_mas_template_type_add->TemplateName->caption() ?><?php echo $ivf_mas_template_type_add->TemplateName->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $ivf_mas_template_type_add->RightColumnClass ?>"><div <?php echo $ivf_mas_template_type_add->TemplateName->cellAttributes() ?>>
<span id="el_ivf_mas_template_type_TemplateName">
<input type="text" data-table="ivf_mas_template_type" data-field="x_TemplateName" name="x_TemplateName" id="x_TemplateName" size="30" maxlength="45" placeholder="<?php echo HtmlEncode($ivf_mas_template_type_add->TemplateName->getPlaceHolder()) ?>" value="<?php echo $ivf_mas_template_type_add->TemplateName->EditValue ?>"<?php echo $ivf_mas_template_type_add->TemplateName->editAttributes() ?>>
</span>
<?php echo $ivf_mas_template_type_add->TemplateName->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
