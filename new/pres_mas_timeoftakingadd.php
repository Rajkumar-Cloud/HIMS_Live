...
<script>
var fpres_mas_timeoftakingadd, currentPageID;
loadjs.ready("head", function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($pres_mas_timeoftaking_add->Time_Of_Taking->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pres_mas_timeoftaking_add->Time_Of_Taking->caption(), $pres_mas_timeoftaking_add->Time_Of_Taking->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fpres_mas_timeoftakingadd.validateRequired = <?php echo Config("CLIENT_VALIDATE") ? "true" : "false" ?>;
...
<?php if ($pres_mas_timeoftaking_add->Time_Of_Taking->Visible) { // Time Of Taking ?>
	<div id="r_Time_Of_Taking" class="form-group row">
		<label id="elh_pres_mas_timeoftaking_Time_Of_Taking" for="x_Time_Of_Taking" class="<?php echo $pres_mas_timeoftaking_add->LeftColumnClass ?>"><?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->caption() ?><?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_mas_timeoftaking_add->RightColumnClass ?>"><div <?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->cellAttributes() ?>>
<span id="el_pres_mas_timeoftaking_Time_Of_Taking">
<input type="text" data-table="pres_mas_timeoftaking" data-field="x_Time_Of_Taking" name="x_Time_Of_Taking" id="x_Time_Of_Taking" size="30" maxlength="45" placeholder="<?php echo HtmlEncode($pres_mas_timeoftaking_add->Time_Of_Taking->getPlaceHolder()) ?>" value="<?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->EditValue ?>"<?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->editAttributes() ?>>
</span>
<?php echo $pres_mas_timeoftaking_add->Time_Of_Taking->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
