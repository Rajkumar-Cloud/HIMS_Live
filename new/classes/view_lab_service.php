 ...
 */
class view_lab_service extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function getDetailUrl()
	{
		...
		if ($this->getCurrentDetailTable() == "view_lab_service_sub") {
			$detailUrl = $GLOBALS["view_lab_service_sub"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_CODE=" . urlencode($this->CODE->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "view_lab_servicelist.php";
		return $detailUrl;
	}
	...
	public function getSqlWhere() // Where
	{
		...
		$this->TableFilter = "`DEPARTMENT` IN ('LAB','OTR')";
	...
	{
		return ($this->SqlOrderBy != "") ? $this->SqlOrderBy : "`TestSubCd` ASC";
	}
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["view_lab_service_sub"]))
				$GLOBALS["view_lab_service_sub"] = new view_lab_service_sub();
			$rswrk = $GLOBALS["view_lab_service_sub"]->loadRs("`CODE` = " . QuotedValue($rsold['CODE'], DATATYPE_STRING, 'DB'));
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["view_lab_service_sub"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["view_lab_service_sub"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["view_lab_service_sub"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		if (!isset($GLOBALS["view_lab_service_sub"]))
			$GLOBALS["view_lab_service_sub"] = new view_lab_service_sub();
		$rscascade = $GLOBALS["view_lab_service_sub"]->loadRs("`CODE` = " . QuotedValue($rs['CODE'], DATATYPE_STRING, "DB"));
		$dtlrows = ($rscascade) ? $rscascade->getRows() : [];

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["view_lab_service_sub"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["view_lab_service_sub"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["view_lab_service_sub"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "view_lab_servicelist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "view_lab_serviceedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "view_lab_serviceadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("view_lab_serviceview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_lab_serviceview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "view_lab_serviceadd.php?" . $this->getUrlParm($parm);
		else
			$url = "view_lab_serviceadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("view_lab_serviceedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_lab_serviceedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("view_lab_serviceadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_lab_serviceadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("view_lab_servicedelete.php", $this->getUrlParm());
	}
