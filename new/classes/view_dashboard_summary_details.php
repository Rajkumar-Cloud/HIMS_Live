 ...
 */
class view_dashboard_summary_details extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// UserName
		$this->UserName = new DbField('view_dashboard_summary_details', 'view_dashboard_summary_details', 'x_UserName', 'UserName', '`UserName`', '`UserName`', 200, 45, -1, FALSE, '`UserName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UserName->IsForeignKey = TRUE; // Foreign key field
		$this->UserName->Sortable = TRUE; // Allow sort
		$this->fields['UserName'] = &$this->UserName;

		// sum(Amount)
		$this->sum28Amount29 = new DbField('view_dashboard_summary_details', 'view_dashboard_summary_details', 'x_sum28Amount29', 'sum(Amount)', '`sum(Amount)`', '`sum(Amount)`', 131, 34, -1, FALSE, '`sum(Amount)`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		if ($this->getCurrentDetailTable() == "view_dashboard_summary_modeofpayment_details") {
			$detailUrl = $GLOBALS["view_dashboard_summary_modeofpayment_details"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
		...
		if ($detailUrl == "")
			$detailUrl = "view_dashboard_summary_detailslist.php";
		return $detailUrl;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "view_dashboard_summary_detailslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "view_dashboard_summary_detailsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "view_dashboard_summary_detailsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "view_dashboard_summary_detailsadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("view_dashboard_summary_detailsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_dashboard_summary_detailsview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "view_dashboard_summary_detailsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "view_dashboard_summary_detailsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_dashboard_summary_detailsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_dashboard_summary_detailsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("view_dashboard_summary_detailsdelete.php", $this->getUrlParm());
	}
