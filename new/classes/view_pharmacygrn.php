...
class view_pharmacygrn extends DbTable
{
	...
	public function __construct()
	{
		...
		// PrName
		$this->PrName = new DbField('view_pharmacygrn', 'view_pharmacygrn', 'x_PrName', 'PrName', '`PrName`', '`PrName`', 200, 100, -1, FALSE, '`EV__PrName`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		$this->PrName->Required = TRUE; // Required field
		$this->PrName->Sortable = TRUE; // Allow sort
		$this->PrName->Lookup = new Lookup('PrName', 'pharmacy_storemast', FALSE, 'DES', ["DES","","",""], [], [], [], [], ["PRC","BATCHNO","EXPDT","MFRCODE","MRP","PurValue","PSGST","PCGST","RT","SaleRate","SSGST","SCGST"], ["x_PRC","x_BatchNo","x_ExpDate","x_MFRCODE","x_MRP","x_PurValue","x_PSGST","x_PCGST","x_ItemValue","x_SalRate","x_SSGST","x_SCGST"], '', '');
	...
	public function getViewUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("view_pharmacygrnview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		...
		// To cancel, set return value to FALSE
				...
				$cid = $_GET["fk_id"] ;
						...
						$sql = "SELECT * FROM ganeshkumar_bjhims.pharmacy_batchmas where PRC='".$rsnew["PRC"]."' and BATCHNO='".$rsnew["BatchNo"]."' and `BRCODE`='".$rsnew["BRCODE"]."' and  `HospID`='".HospitalID()."'; ";
								...
								if($rsnew["ExpDate"] == null)
								{
								$sqlC = "UPDATE `ganeshkumar_bjhims`.`pharmacy_storemast` SET `BRCODE`='".$rsnew["BRCODE"]."', `RT`='".$rsnew["SalRate"]."',
