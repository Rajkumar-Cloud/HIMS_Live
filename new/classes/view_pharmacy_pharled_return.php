...
class view_pharmacy_pharled_return extends DbTable
{
	...
	public function __construct()
	{
		...
		// Product
		$this->Product = new DbField('view_pharmacy_pharled_return', 'view_pharmacy_pharled_return', 'x_Product', 'Product', '`Product`', '`Product`', 200, 100, -1, FALSE, '`EV__Product`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		$this->Product->Required = TRUE; // Required field
		$this->Product->Sortable = TRUE; // Allow sort
		$this->Product->Lookup = new Lookup('Product', 'view_pharmacy_search_product', FALSE, 'DES', ["DES","Stock","BATCH","EXPDT"], [], [], [], [], ["RT","MFRCODE","EXPDT","BATCH","BRCODE","PRC","Stock","MFRCODE"], ["x_RT","x_Mfg","x_EXPDT","x_BATCHNO","x_BRCODE","x_PRC","x_UR","x_MFRCODE"], '`EXPDT` ASC', '');
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`DES`, ''),'" . ValueSeparator(1, $this->Product) . "',COALESCE(`Stock`,''),'" . ValueSeparator(2, $this->Product) . "',COALESCE(`BATCH`,''),'" . ValueSeparator(3, $this->Product) . "',COALESCE(`EXPDT`,'')) FROM `view_pharmacy_search_product` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`DES` = `view_pharmacy_pharled_return`.`Product` LIMIT 1) AS `EV__Product` FROM `view_pharmacy_pharled_return`" .
			") `TMP_TABLE`";
	...
	public function getViewUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("view_pharmacy_pharled_returnview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
