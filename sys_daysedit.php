...
var fsys_daysedit = currentForm = new ew.Form("fsys_daysedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($sys_days_edit->id->Required) { ?>
			...
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $sys_days->id->caption(), $sys_days->id->RequiredErrorMessage)) ?>");
		<?php } ?>
...
fsys_daysedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php if ($sys_days_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $sys_days_edit->Token ?>">
<?php } ?>
...
<?php if ($sys_days->id->Visible) { // id ?>
	<div id="r_id" class="form-group row">
		<label id="elh_sys_days_id" class="<?php echo $sys_days_edit->LeftColumnClass ?>"><?php echo $sys_days->id->caption() ?><?php echo ($sys_days->id->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sys_days_edit->RightColumnClass ?>"><div<?php echo $sys_days->id->cellAttributes() ?>>
<span id="el_sys_days_id">
<span<?php echo $sys_days->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($sys_days->id->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="sys_days" data-field="x_id" name="x_id" id="x_id" value="<?php echo HtmlEncode($sys_days->id->CurrentValue) ?>">
<?php echo $sys_days->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($sys_days->Days->Visible) { // Days ?>
	<div id="r_Days" class="form-group row">
		<label id="elh_sys_days_Days" for="x_Days" class="<?php echo $sys_days_edit->LeftColumnClass ?>"><?php echo $sys_days->Days->caption() ?><?php echo ($sys_days->Days->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sys_days_edit->RightColumnClass ?>"><div<?php echo $sys_days->Days->cellAttributes() ?>>
