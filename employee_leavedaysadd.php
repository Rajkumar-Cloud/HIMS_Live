...
// Create page object
$employee_leavedays_add = new employee_leavedays_add();

// Run the page
$employee_leavedays_add->run();
...
$employee_leavedays_add->Page_Render();
?>
...
var femployee_leavedaysadd = currentForm = new ew.Form("femployee_leavedaysadd", "add");
...
femployee_leavedaysadd.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($employee_leavedays_add->employee_leave->Required) { ?>
		...
		<?php if ($employee_leavedays_add->leave_date->Required) { ?>
		...
		<?php if ($employee_leavedays_add->leave_type->Required) { ?>
...
femployee_leavedaysadd.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
femployee_leavedaysadd.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
femployee_leavedaysadd.lists["x_leave_type"] = <?php echo $employee_leavedays_add->leave_type->Lookup->toClientList() ?>;
femployee_leavedaysadd.lists["x_leave_type"].options = <?php echo JsonEncode($employee_leavedays_add->leave_type->options(FALSE, TRUE)) ?>;
...
<?php $employee_leavedays_add->showPageHeader(); ?>
<?php
$employee_leavedays_add->showMessage();
?>
<form name="femployee_leavedaysadd" id="femployee_leavedaysadd" class="<?php echo $employee_leavedays_add->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($employee_leavedays_add->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $employee_leavedays_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="employee_leavedays">
<input type="hidden" name="action" id="action" value="insert">
<input type="hidden" name="modal" value="<?php echo (int)$employee_leavedays_add->IsModal ?>">
<div class="ew-add-div"><!-- page* -->
<?php if ($employee_leavedays->employee_leave->Visible) { // employee_leave ?>
	<div id="r_employee_leave" class="form-group row">
		<label id="elh_employee_leavedays_employee_leave" for="x_employee_leave" class="<?php echo $employee_leavedays_add->LeftColumnClass ?>"><?php echo $employee_leavedays->employee_leave->caption() ?><?php echo ($employee_leavedays->employee_leave->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $employee_leavedays_add->RightColumnClass ?>"><div<?php echo $employee_leavedays->employee_leave->cellAttributes() ?>>
...
<?php if ($employee_leavedays->leave_date->Visible) { // leave_date ?>
	<div id="r_leave_date" class="form-group row">
		<label id="elh_employee_leavedays_leave_date" for="x_leave_date" class="<?php echo $employee_leavedays_add->LeftColumnClass ?>"><?php echo $employee_leavedays->leave_date->caption() ?><?php echo ($employee_leavedays->leave_date->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $employee_leavedays_add->RightColumnClass ?>"><div<?php echo $employee_leavedays->leave_date->cellAttributes() ?>>
...
<?php if ($employee_leavedays->leave_type->Visible) { // leave_type ?>
	<div id="r_leave_type" class="form-group row">
		<label id="elh_employee_leavedays_leave_type" class="<?php echo $employee_leavedays_add->LeftColumnClass ?>"><?php echo $employee_leavedays->leave_type->caption() ?><?php echo ($employee_leavedays->leave_type->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $employee_leavedays_add->RightColumnClass ?>"><div<?php echo $employee_leavedays->leave_type->cellAttributes() ?>>
...
<?php if (!$employee_leavedays_add->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $employee_leavedays_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("AddBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $employee_leavedays_add->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$employee_leavedays_add->showPageFooter();
if (DEBUG_ENABLED)
...
<?php
$employee_leavedays_add->terminate();
?>