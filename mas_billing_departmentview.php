<?php
namespace PHPMaker2019\HIMS;

// Session
if (session_status() !== PHP_SESSION_ACTIVE)
	session_start(); // Init session data

// Output buffering
ob_start(); 

// Autoload
include_once "autoload.php";
?>
<?php

// Write header
WriteHeader(FALSE);

// Create page object
$mas_billing_department_view = new mas_billing_department_view();

// Run the page
$mas_billing_department_view->run();

// Setup login status
SetupLoginStatus();
SetClientVar("login", LoginStatus());

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$mas_billing_department_view->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if (!$mas_billing_department->isExport()) { ?>
<script>

// Form object
currentPageID = ew.PAGE_ID = "view";
var fmas_billing_departmentview = currentForm = new ew.Form("fmas_billing_departmentview", "view");

// Form_CustomValidate event
fmas_billing_departmentview.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!

	// Your custom validation code here, return false if invalid.
	return true;
}

// Use JavaScript validation or not
fmas_billing_departmentview.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
// Form object for search

</script>
<script>

// Write your client script here, no need to add script tags.
</script>
<?php } ?>
<?php if (!$mas_billing_department->isExport()) { ?>
<div class="btn-toolbar ew-toolbar">
<?php $mas_billing_department_view->ExportOptions->render("body") ?>
<?php $mas_billing_department_view->OtherOptions->render("body") ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $mas_billing_department_view->showPageHeader(); ?>
<?php
$mas_billing_department_view->showMessage();
?>
<form name="fmas_billing_departmentview" id="fmas_billing_departmentview" class="form-inline ew-form ew-view-form" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($mas_billing_department_view->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $mas_billing_department_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="mas_billing_department">
<input type="hidden" name="modal" value="<?php echo (int)$mas_billing_department_view->IsModal ?>">
<table class="table table-striped table-sm ew-view-table">
<?php if ($mas_billing_department->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="<?php echo $mas_billing_department_view->TableLeftColumnClass ?>"><span id="elh_mas_billing_department_id"><?php echo $mas_billing_department->id->caption() ?></span></td>
		<td data-name="id"<?php echo $mas_billing_department->id->cellAttributes() ?>>
<span id="el_mas_billing_department_id">
<span<?php echo $mas_billing_department->id->viewAttributes() ?>>
<?php echo $mas_billing_department->id->getViewValue() ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($mas_billing_department->department->Visible) { // department ?>
	<tr id="r_department">
		<td class="<?php echo $mas_billing_department_view->TableLeftColumnClass ?>"><span id="elh_mas_billing_department_department"><?php echo $mas_billing_department->department->caption() ?></span></td>
		<td data-name="department"<?php echo $mas_billing_department->department->cellAttributes() ?>>
<span id="el_mas_billing_department_department">
<span<?php echo $mas_billing_department->department->viewAttributes() ?>>
<?php echo $mas_billing_department->department->getViewValue() ?></span>
</span>
</td>
	</tr>
<?php } ?>
</table>
</form>
<?php
$mas_billing_department_view->showPageFooter();
if (DEBUG_ENABLED)
	echo GetDebugMessage();
?>
<?php if (!$mas_billing_department->isExport()) { ?>
<script>

// Write your table-specific startup script here
// document.write("page loaded");

</script>
<?php } ?>
<?php include_once "footer.php" ?>
<?php
$mas_billing_department_view->terminate();
?>