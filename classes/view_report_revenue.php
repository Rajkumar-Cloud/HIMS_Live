 ...
 */
class view_report_revenue extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// Date
		$this->Date = new DbField('view_report_revenue', 'view_report_revenue', 'x_Date', 'Date', '`Date`', CastDateFieldForLike('`Date`', 7, "DB"), 133, 7, FALSE, '`Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// BillType
		$this->BillType = new DbField('view_report_revenue', 'view_report_revenue', 'x_BillType', 'BillType', '`BillType`', '`BillType`', 200, -1, FALSE, '`BillType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->BillType->Sortable = TRUE; // Allow sort
		$this->fields['BillType'] = &$this->BillType;

		// TotalAmount
		$this->TotalAmount = new DbField('view_report_revenue', 'view_report_revenue', 'x_TotalAmount', 'TotalAmount', '`TotalAmount`', '`TotalAmount`', 131, -1, FALSE, '`TotalAmount`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// HospID
		$this->HospID = new DbField('view_report_revenue', 'view_report_revenue', 'x_HospID', 'HospID', '`HospID`', '`HospID`', 3, -1, FALSE, '`HospID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "view_report_revenuelist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "view_report_revenueview.php")
			return $Language->phrase("View");
		elseif ($pageName == "view_report_revenueedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "view_report_revenueadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("view_report_revenueview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("view_report_revenueview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "view_report_revenueadd.php?" . $this->getUrlParm($parm);
		else
			$url = "view_report_revenueadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_report_revenueedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("view_report_revenueadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("view_report_revenuedelete.php", $this->getUrlParm());
	}
