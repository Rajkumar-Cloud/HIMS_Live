...
class userlevelpermissions extends DbTable
{
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('userlevelpermissions', 'userlevelpermissions', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->id->Lookup = new Lookup('id', 'userlevels', FALSE, 'id', ["UserLevelsName","","",""], [], [], [], [], [], [], '', '');
		...
		// Permissions
		$this->Permissions = new DbField('userlevelpermissions', 'userlevelpermissions', 'x_Permissions', 'Permissions', '`Permissions`', '`Permissions`', 3, -1, FALSE, '`Permissions`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Permissions->Required = TRUE; // Required field
		$this->Permissions->Sortable = TRUE; // Allow sort
		$this->Permissions->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->Permissions);
				} else {
					$doc->exportCaption($this->id);
					$doc->exportCaption($this->_tableName);
					$doc->exportCaption($this->Permissions);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->Permissions);
					} else {
						$doc->exportField($this->id);
						$doc->exportField($this->_tableName);
						$doc->exportField($this->Permissions);
