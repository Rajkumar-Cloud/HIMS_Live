...
/**
 * Table class for vitals
 */
class vitals extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vitals';
		$this->TableName = 'vitals';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`vitals`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('vitals', 'vitals', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Reception
		$this->Reception = new DbField('vitals', 'vitals', 'x_Reception', 'Reception', '`Reception`', '`Reception`', 3, -1, FALSE, '`Reception`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PatientId
		$this->PatientId = new DbField('vitals', 'vitals', 'x_PatientId', 'PatientId', '`PatientId`', '`PatientId`', 200, -1, FALSE, '`PatientId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PatientId->IsForeignKey = TRUE; // Foreign key field
		$this->PatientId->Sortable = TRUE; // Allow sort
		$this->fields['PatientId'] = &$this->PatientId;

		// PatientName
		$this->PatientName = new DbField('vitals', 'vitals', 'x_PatientName', 'PatientName', '`PatientName`', '`PatientName`', 200, -1, FALSE, '`PatientName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PatientName->IsForeignKey = TRUE; // Foreign key field
		$this->PatientName->Sortable = TRUE; // Allow sort
		$this->fields['PatientName'] = &$this->PatientName;

		// HT
		$this->HT = new DbField('vitals', 'vitals', 'x_HT', 'HT', '`HT`', '`HT`', 200, -1, FALSE, '`HT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->HT->Sortable = TRUE; // Allow sort
		$this->fields['HT'] = &$this->HT;

		// WT
		$this->WT = new DbField('vitals', 'vitals', 'x_WT', 'WT', '`WT`', '`WT`', 200, -1, FALSE, '`WT`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->WT->Sortable = TRUE; // Allow sort
		$this->fields['WT'] = &$this->WT;

		// BP
		$this->BP = new DbField('vitals', 'vitals', 'x_BP', 'BP', '`BP`', '`BP`', 200, -1, FALSE, '`BP`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->BP->Sortable = TRUE; // Allow sort
		$this->fields['BP'] = &$this->BP;

		// PULSE
		$this->PULSE = new DbField('vitals', 'vitals', 'x_PULSE', 'PULSE', '`PULSE`', '`PULSE`', 200, -1, FALSE, '`PULSE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "reception") {
			if ($this->PatientName->getSessionValue() <> "")
				$masterFilter .= "`PatientName`=" . QuotedValue($this->PatientName->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
			if ($this->PatientId->getSessionValue() <> "")
				$masterFilter .= " AND `PatientID`=" . QuotedValue($this->PatientId->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "reception") {
			if ($this->PatientName->getSessionValue() <> "")
				$detailFilter .= "`PatientName`=" . QuotedValue($this->PatientName->getSessionValue(), DATATYPE_STRING, "DB");
			else
				return "";
			if ($this->PatientId->getSessionValue() <> "")
	...
	// Master filter
	public function sqlMasterFilter_reception()
	{
		return "`PatientName`='@PatientName@' AND `PatientID`='@PatientID@' AND `id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_reception()
	{
		return "`PatientName`='@PatientName@' AND `PatientId`='@PatientId@' AND `Reception`=@Reception@";
	}

	// Table level SQL
	public function getSqlFrom() // From
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vitals`";
	}
	...
	{
		global $Language;
		if ($pageName == "vitalsview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vitalsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vitalsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vitalsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vitalsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vitalsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vitalsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vitalsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vitalsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vitalsdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "reception" && !ContainsString($url, TABLE_SHOW_MASTER . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_PatientName=" . urlencode($this->PatientName->CurrentValue);
			$url .= "&fk_PatientID=" . urlencode($this->PatientId->CurrentValue);
