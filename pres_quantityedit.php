...
var fpres_quantityedit = currentForm = new ew.Form("fpres_quantityedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($pres_quantity_edit->id->Required) { ?>
			...
			if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
				return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $pres_quantity->id->caption(), $pres_quantity->id->RequiredErrorMessage)) ?>");
		<?php } ?>
...
fpres_quantityedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php if ($pres_quantity_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $pres_quantity_edit->Token ?>">
<?php } ?>
...
<?php if ($pres_quantity->id->Visible) { // id ?>
	<div id="r_id" class="form-group row">
		<label id="elh_pres_quantity_id" class="<?php echo $pres_quantity_edit->LeftColumnClass ?>"><?php echo $pres_quantity->id->caption() ?><?php echo ($pres_quantity->id->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_quantity_edit->RightColumnClass ?>"><div<?php echo $pres_quantity->id->cellAttributes() ?>>
<span id="el_pres_quantity_id">
<span<?php echo $pres_quantity->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?php echo RemoveHtml($pres_quantity->id->EditValue) ?>"></span>
</span>
<input type="hidden" data-table="pres_quantity" data-field="x_id" name="x_id" id="x_id" value="<?php echo HtmlEncode($pres_quantity->id->CurrentValue) ?>">
<?php echo $pres_quantity->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pres_quantity->Quantity->Visible) { // Quantity ?>
	<div id="r_Quantity" class="form-group row">
		<label id="elh_pres_quantity_Quantity" for="x_Quantity" class="<?php echo $pres_quantity_edit->LeftColumnClass ?>"><?php echo $pres_quantity->Quantity->caption() ?><?php echo ($pres_quantity->Quantity->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_quantity_edit->RightColumnClass ?>"><div<?php echo $pres_quantity->Quantity->cellAttributes() ?>>
...
<?php if ($pres_quantity->Value->Visible) { // Value ?>
	<div id="r_Value" class="form-group row">
		<label id="elh_pres_quantity_Value" for="x_Value" class="<?php echo $pres_quantity_edit->LeftColumnClass ?>"><?php echo $pres_quantity->Value->caption() ?><?php echo ($pres_quantity->Value->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $pres_quantity_edit->RightColumnClass ?>"><div<?php echo $pres_quantity->Value->cellAttributes() ?>>
